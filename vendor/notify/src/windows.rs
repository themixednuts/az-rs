...
fn dropped_watch_event(path: &Path) -> Event {
    Event::new(EventKind::Other)
        .set_flag(Flag::Rescan)
        .add_path(path.to_path_buf())
}

unsafe fn report_dropped_watch(request: &ReadDirectoryRequest) {
    // Queue removal before notifying the consumer. A consumer may respond to
    // the rescan event by re-arming immediately; preserving this order keeps
    // the stale removal from racing behind the replacement arm.
    request.unwatch();
    emit_event(
        &request.event_handler,
        Ok(dropped_watch_event(&request.data.dir)),
    );
    unsafe {
...
mod tests {
    ...
    #[test]
    fn dropped_watch_event_requests_a_rescan_for_the_watched_root() {
        let path = PathBuf::from(r"C:\workspace\assets");
        let event = dropped_watch_event(&path);

        assert!(event.need_rescan());
        assert_eq!(event.paths, [path]);
    }
}
