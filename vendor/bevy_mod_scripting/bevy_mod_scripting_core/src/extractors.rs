...
use bevy_ecs::query::{Access, AccessConflicts, ComponentIdSet};
...
fn individual_conflicts(conflicts: AccessConflicts) -> ComponentIdSet {
    match conflicts {
        ...
        AccessConflicts::Individual(component_ids) => component_ids,
    }
}
...
pub(crate) fn get_all_access_ids(access: &Access) -> Vec<(ReflectAccessId, bool)> {
    ...
    for c in read.iter() {
        result.push((ReflectAccessId::for_component_id(c), false));
    }
    for c in written.iter() {
        result.push((ReflectAccessId::for_component_id(c), true));
    }
