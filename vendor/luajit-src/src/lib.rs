...
impl Build {
    ...
    pub fn build_msvc(&mut self) -> Artifacts {
        ...
        let mut cl_flags = env::var_os("CL").unwrap_or_default();
        if !cl_flags.is_empty() {
            cl_flags.push(" ");
        }
        cl_flags.push(msvc_crt_flag(&target_features));
        msvcbuild.env("CL", cl_flags);
...
#[cfg(test)]
mod tests {
    use super::msvc_crt_flag;

    #[test]
    fn msvc_runtime_follows_the_rust_target_feature() {
        assert_eq!(msvc_crt_flag(""), "/MD");
        assert_eq!(msvc_crt_flag("sse2,crt-static"), "/MT");
        assert_eq!(msvc_crt_flag("crt-static-extra"), "/MD");
    }
}
