...
use crate::{ElementExt, tab::TabVariant};
...
/// Horizontal extent used when rendering a bottom dock together with left and
/// right docks.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum BottomDockExtent {
    /// The bottom dock is constrained between the left and right docks.
    BetweenLeftAndRight,
    /// The bottom dock extends under the left dock and stops at the right dock.
    BoundedByRight,
    /// The bottom dock starts after the left dock and extends under the right dock.
    BoundedByLeft,
    /// The bottom dock spans the full dock area width.
    FullWidth,
}

impl Default for BottomDockExtent {
    fn default() -> Self {
        Self::BetweenLeftAndRight
    }
}
...
pub struct DockArea {
    ...
    /// How the bottom dock is bounded relative to left and right docks.
    bottom_dock_extent: BottomDockExtent,
    ...
    /// Whether to show the stock zoom and overflow actions in tab-panel title bars.
    tab_panel_actions_visible: bool,
    /// Tab variant used by tab panels in this dock area.
...
impl DockArea {
    pub fn new(
        ...
        let mut this = Self {
            ...
            right_dock: None,
            bottom_dock: None,
            bottom_dock_extent: BottomDockExtent::default(),
            zoom_view: None,
            toggle_button_panels: Edges::default(),
            toggle_button_visible: true,
            tab_panel_actions_visible: true,
    ...
    /// Set the horizontal extent of the bottom dock relative to side docks.
    pub fn set_bottom_dock_extent(&mut self, extent: BottomDockExtent, cx: &mut Context<Self>) {
        self.bottom_dock_extent = extent;
        cx.notify();
    }
    ...
    /// Set the tab variant used by every tab panel in this dock area.
    pub fn tab_variant(mut self, variant: TabVariant) -> Self {
        self.tab_variant = variant;
        self
    }

    /// Set whether stock zoom and overflow actions are shown in tab-panel title bars.
    ///
    /// Panel-provided toolbar buttons and title suffixes are unaffected.
    pub fn tab_panel_actions_visible(mut self, visible: bool) -> Self {
        self.tab_panel_actions_visible = visible;
        self
    }

    /// Set whether dock collapse/expand toggle buttons are shown in tab-panel title bars.
            ...
            .map(|this| {
                if let Some(zoom_view) = self.zoom_view.clone() {
                    ...
                    match &self.center {
                        ...
                        _ => {
                            // render dock
                            match self.bottom_dock_extent {
                                BottomDockExtent::BoundedByRight => this.child(
                                    div()
                                        .flex()
                                        .flex_row()
                                        .h_full()
                                ...
                                BottomDockExtent::BoundedByLeft => this.child(
                                    div()
                                        .flex()
                                        .flex_row()
                                        .h_full()
                                ...
                                BottomDockExtent::FullWidth => this.child(
                                    div()
                                        .flex()
                                        .flex_col()
                                        .h_full()
                                ...
                                BottomDockExtent::BetweenLeftAndRight => this.child(
                                    div()
                                        .flex()
                                        .flex_row()
                                        .h_full()
