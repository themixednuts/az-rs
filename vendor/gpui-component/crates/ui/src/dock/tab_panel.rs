...
static DOCK_TAB_ACTIVATION_PERF_SINK: OnceLock<fn(&'static str)> = OnceLock::new();

/// Install the editor's allocation-free dock-tab interaction start hook.
pub fn set_dock_tab_activation_perf_sink(sink: fn(&'static str)) {
    let _ = DOCK_TAB_ACTIVATION_PERF_SINK.set(sink);
}

fn record_dock_tab_activation(target_panel: &'static str) {
    if let Some(sink) = DOCK_TAB_ACTIVATION_PERF_SINK.get() {
        sink(target_panel);
    }
}
...
impl TabPanel {
    ...
    fn render_toolbar(
        ...
        let actions_visible = self.dock_area.upgrade().map_or(true, |dock_area| {
            dock_area.read(cx).tab_panel_actions_visible
        });
    ...
    fn render_title_bar(
        ...
        let tab_variant = dock_area.read(cx).tab_variant;
        let tab_bar_background = if tab_variant == crate::tab::TabVariant::Dock {
            cx.theme().tab
        } else {
            cx.theme().tab_bar
        };
        ...
        TabBar::new("tab-bar")
            ...
            .children(self.panels.iter().enumerate().filter_map(|(ix, panel)| {
                ...
                let target_panel_name = panel.panel_name(cx);
                    ...
                    Tab::new()
                        ...
                        .on_click(cx.listener({
                            ...
                            move |view, _, window, cx| {
                                if ix != view.active_ix {
                                    record_dock_tab_activation(target_panel_name);
                                }
