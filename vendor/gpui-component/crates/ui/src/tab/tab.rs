...
pub enum TabVariant {
    #[default]
    Tab,
    /// Compact dock chrome with an active-surface fill and accent underline.
    Dock,
    Outline,
    Pill,
    Segmented,
...
impl TabVariant {
    ...
    pub(super) fn inner_height(&self, size: Size) -> Pixels {
        match size {
            Size::XSmall => match self {
                TabVariant::Tab | TabVariant::Dock | TabVariant::Outline | TabVariant::Pill => {
                    px(18.)
                }
            ...
            _ => match self {
                TabVariant::Tab | TabVariant::Dock => px(30.),
