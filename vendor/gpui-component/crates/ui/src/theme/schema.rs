...
pub struct ThemeConfigColors {
    ...
    /// Neutral authored/runtime type glyph.
    #[serde(rename = "type_accent.neutral")]
    pub type_accent_neutral: Option<SharedString>,
    /// Slate authored/runtime type glyph.
    #[serde(rename = "type_accent.slate")]
    pub type_accent_slate: Option<SharedString>,
    /// Level/scene type glyph.
    #[serde(rename = "type_accent.level")]
    pub type_accent_level: Option<SharedString>,
    /// Prefab/UI type glyph.
    #[serde(rename = "type_accent.prefab")]
    pub type_accent_prefab: Option<SharedString>,
    /// Material/slice/sun type glyph.
    #[serde(rename = "type_accent.gold")]
    pub type_accent_gold: Option<SharedString>,
    /// Light type glyph.
    #[serde(rename = "type_accent.light")]
    pub type_accent_light: Option<SharedString>,
    /// Script/volume type glyph.
    #[serde(rename = "type_accent.teal")]
    pub type_accent_teal: Option<SharedString>,
    /// Terrain/texture type glyph.
    #[serde(rename = "type_accent.terrain")]
    pub type_accent_terrain: Option<SharedString>,
    /// Audio type glyph.
    #[serde(rename = "type_accent.audio")]
    pub type_accent_audio: Option<SharedString>,
    /// Animation type glyph.
    #[serde(rename = "type_accent.animation")]
    pub type_accent_animation: Option<SharedString>,
    ...
    /// Background color for recessed inputs such as Input and Select.
...
impl ThemeColor {
    ...
    pub(crate) fn apply_config(&mut self, config: &ThemeConfig, default_theme: &ThemeColor) {
        ...
        apply_color!(type_accent_neutral, fallback = self.muted_foreground);
        apply_color!(type_accent_slate, fallback = self.muted_foreground);
        apply_color!(type_accent_level, fallback = self.warning);
        apply_color!(type_accent_prefab, fallback = self.magenta_light);
        apply_color!(type_accent_gold, fallback = self.warning);
        apply_color!(type_accent_light, fallback = self.yellow_light);
        apply_color!(type_accent_teal, fallback = self.cyan);
        apply_color!(type_accent_terrain, fallback = self.green_light);
        apply_color!(type_accent_audio, fallback = self.magenta_light);
        apply_color!(type_accent_animation, fallback = self.green_light);
        ...
        apply_color!(
            ...
            fallback = if config.mode.is_dark() {
                self.input.mix_oklab(Hsla::transparent_black(), 0.3)
            } else {
                self.background
            }
        );
