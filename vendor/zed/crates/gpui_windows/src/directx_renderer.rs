...
use crate::composition_tree::DirectComposition;
...
struct DirectXResources {
    ...
    _frame_latency_waitable: FrameLatencyWaitable,
    ...
    present_history: [PresentSubmission; PRESENT_HISTORY_CAPACITY],
    present_history_next: usize,
    last_displayed_present: u32,
    previous_frame_statistics: Option<DXGI_FRAME_STATISTICS>,
    qpc_ticks_per_refresh: f64,
    qpc_frequency: i64,
    media_swap_chain: Option<IDXGISwapChainMedia>,
    frame_statistics_failure_reported: bool,
    allow_tearing: bool,
}
...
struct DirectXRenderPipelines {
    ...
    composition_hole_pipeline: PipelineState<Quad>,
...
impl DirectXRenderer {
    ...
    pub(crate) fn viewport_visual_slot(&self, window_id: u64) -> Option<crate::ViewportVisualSlot> {
        self.direct_composition
            .as_ref()
            .map(|composition| composition.viewport_visual_slot(window_id))
    }
    ...
    fn present(&mut self) -> Result<()> {
        let _present_perf = FramePerfGuard::new("frame.present_api");
        let resources = self.resources.as_mut().expect("resources missing");
        let mut submitted_qpc = 0;
        unsafe { QueryPerformanceCounter(&mut submitted_qpc) }?;
        // In the split DComp compositor, GPUI content and the Bevy visual's
        // transform/clip belong to the same composition batch. Allow-tearing
        // publishes GPUI's new layout markers independently and can expose the
        // prior Bevy rectangle for one displayed composition. Bevy retains its
        // own independent low-latency policy; only GPUI's chrome swapchain is
        // submitted as an ordinary composition present here.
        let present_flags = if resources.allow_tearing && self.direct_composition.is_none() {
            DXGI_PRESENT_ALLOW_TEARING
        } else {
            DXGI_PRESENT(0)
        };
        let result = unsafe { resources.swap_chain.Present(0, present_flags) };
        result.ok().context("Presenting swap chain failed")?;
        if let Ok(present_id) = unsafe { resources.swap_chain.GetLastPresentCount() } {
            resources.record_present_submission(present_id, submitted_qpc);
        }
        resources.record_displayed_frame_statistics();
        crate::viewport_bridge::record_immediate_frame_present_submitted();
        Ok(())
    }
    ...
    pub(crate) fn draw(
        ...
        let _paint_perf = FramePerfGuard::new("frame.gpui_paint");
        if self.skip_draws {
        ...
        let mut composition_hole_seen = false;
        ...
        for batch in scene.batches() {
            match batch {
                ...
                PrimitiveBatch::CompositionHoles(range) => {
                    if self.direct_composition.is_some() {
                        composition_hole_seen = true;
                        self.stage_composition_holes(&scene.composition_holes[range.clone()])?;
                        self.draw_composition_holes(range.start, range.len())
                    } else {
                        Ok(())
                    }
                }
            ...
            .context(format!(
                "scene too large:\
                {} paths, {} shadows, {} quads, {} holes, {} underlines, {} mono, {} subpixel, {} poly, {} surfaces",
                scene.paths.len(),
                scene.shadows.len(),
                scene.quads.len(),
                scene.composition_holes.len(),
        ...
        if !composition_hole_seen && let Some(composition) = &self.direct_composition {
            composition.hide_composition_hole()?;
        }
        let layout_changed = self
            .direct_composition
            .as_ref()
            .is_some_and(DirectComposition::composition_hole_layout_changed);
        if layout_changed {
            let wait_result = unsafe { DCompositionWaitForCompositorClock(None, 100) };
            if wait_result != WAIT_OBJECT_0.0 {
                anyhow::bail!(
                    "DirectComposition compositor-clock wait failed with result {wait_result}"
                );
            }
            crate::viewport_bridge::record_viewport_perf("dcomp.layout_batch_boundary", 1);
        }
        self.present()?;
        if let Some(composition) = &self.direct_composition {
            // Publish the GPUI buffer containing the new composition hole
            // before committing the matching Bevy visual transform. Both
            // operations run immediately after the compositor-clock boundary,
            // and keeping Commit last prevents a scheduling stall from making
            // the new transform visible with the previous GPUI hole frame.
            composition.commit_composition_batch()?;
            // Surface replacement is rare and owns an explicit acknowledgement:
            // retire the old complete swapchain only after the transaction that
            // attached its replacement has been processed. Layout-only batches
            // never wait here and stay paired with GPUI's Present.
            composition.acknowledge_surface_handoff()?;
        }
        ...
        unsafe {
            resources
                ...
                .ResizeBuffers(
                    ...
                    swap_chain_flags(resources.allow_tearing),
                )
    ...
    fn upload_scene_buffers(&mut self, scene: &Scene) -> Result<()> {
        ...
        if !scene.composition_holes.is_empty() {
            let hole_quads = scene
                .composition_holes
                .iter()
                .map(|hole| Quad {
                    order: hole.order,
                    ...
                    background: rgba(0xffff_ffff).into(),
                    border_color: transparent_black(),
                    corner_radii: hole.corner_radii,
                    border_widths: Edges::default(),
                })
                .collect::<Vec<_>>();
            self.pipelines.composition_hole_pipeline.update_buffer(
                &devices.device,
                &devices.device_context,
                &hole_quads,
            )?;
    ...
    fn draw_composition_holes(&mut self, start: usize, len: usize) -> Result<()> {
        if len == 0 {
            return Ok(());
        }
        let devices = self.devices.as_ref().context("devices missing")?;
        self.pipelines.composition_hole_pipeline.draw_range(
            &devices.device,
            &devices.device_context,
            slice::from_ref(
                &self
    ...
    fn stage_composition_holes(&self, holes: &[CompositionHole]) -> Result<()> {
        let Some(composition) = &self.direct_composition else {
            return Ok(());
        };
        for hole in holes {
            let clipped = hole.bounds.intersect(&hole.content_mask.bounds);
            let left = clipped.origin.x.0.round() as i32;
            let top = clipped.origin.y.0.round() as i32;
            let right = (clipped.origin.x.0 + clipped.size.width.0).round() as i32;
            let bottom = (clipped.origin.y.0 + clipped.size.height.0).round() as i32;
            let rect = crate::ViewportDeviceRect {
                left,
                top,
            ...
            let radii = [
                hole.corner_radii.top_left.0,
                hole.corner_radii.top_right.0,
                hole.corner_radii.bottom_right.0,
                hole.corner_radii.bottom_left.0,
            ];
            composition.update_composition_hole(
                hole.slot_id,
                hole.scene_generation,
                rect,
                radii,
            )?;
            crate::viewport_bridge::record_dcomp_hole_rect(
                hole.slot_id,
                hole.scene_generation,
                rect,
            );
...
impl DirectXResources {
    pub fn new(
        ...
        let tearing_supported = present_tearing_supported(&devices.dxgi_factory);
        let waitable_only = std::env::var("AZOTH_EDITOR_UI_PRESENT_POLICY")
            .is_ok_and(|policy| policy.eq_ignore_ascii_case("waitable"));
        let allow_tearing = tearing_supported && !waitable_only;
        let dcomp_batch = !disable_direct_composition;
        log::info!(
            "Editor low-latency Present policy: {} (tearing supported: {}).",
            if allow_tearing && !dcomp_batch {
                "allow-tearing"
            } else if dcomp_batch {
                "dcomp-batched"
            } else {
        ...
        crate::viewport_bridge::record_viewport_perf(
            "frame.present_tearing_supported",
            u64::from(tearing_supported),
        );
        ...
        let low_latency_swap_chain: IDXGISwapChain2 = swap_chain
            .cast()
            .context("Querying low-latency swap-chain interface")?;
        unsafe { low_latency_swap_chain.SetMaximumFrameLatency(1) }
            .context("Setting one-frame swap-chain latency")?;
        let frame_latency_waitable =
            FrameLatencyWaitable(unsafe { low_latency_swap_chain.GetFrameLatencyWaitableObject() });
        if frame_latency_waitable.0.is_invalid() {
            anyhow::bail!("DXGI waitable swap chain returned a null frame-latency handle");
        }
        let media_swap_chain = swap_chain.cast().ok();

        let mut qpc_frequency = 0;
        unsafe { QueryPerformanceFrequency(&mut qpc_frequency) }
            .context("Querying performance-counter frequency")?;
        log::info!(
            "Configured DXGI waitable flip chain with maximum frame latency 1 ({} buffers).",
            BUFFER_COUNT
        );
        ...
        Ok(Self {
            swap_chain,
            _frame_latency_waitable: frame_latency_waitable,
            ...
            path_intermediate_srv,
            viewport,
            present_history: [PresentSubmission::default(); PRESENT_HISTORY_CAPACITY],
            present_history_next: 0,
            last_displayed_present: 0,
            previous_frame_statistics: None,
            qpc_ticks_per_refresh: 0.0,
            qpc_frequency,
            media_swap_chain,
            frame_statistics_failure_reported: false,
            allow_tearing,
        })
    }

    fn record_present_submission(&mut self, present_id: u32, submitted_qpc: i64) {
        self.present_history[self.present_history_next] = PresentSubmission {
            present_id,
            submitted_qpc,
        };
        self.present_history_next = (self.present_history_next + 1) % PRESENT_HISTORY_CAPACITY;
    }

    fn record_displayed_frame_statistics(&mut self) {
        let mut statistics = DXGI_FRAME_STATISTICS::default();
        let standard_result = unsafe { self.swap_chain.GetFrameStatistics(&mut statistics) };
        if standard_result.is_err() || statistics.PresentCount == 0 {
            let media_result = self.media_swap_chain.as_ref().map(|swap_chain| {
                ...
                let result = unsafe { swap_chain.GetFrameStatisticsMedia(&mut media) };
                if result.is_ok() {
            ...
            if statistics.PresentCount == 0 {
                if !self.frame_statistics_failure_reported {
                    self.frame_statistics_failure_reported = true;
                    log::warn!(
                        "DXGI display statistics unavailable for composition swap chain: standard={standard_result:?}, media={media_result:?}"
                    );
                }
                return;
            }
        ...
        if let Some(previous) = self.previous_frame_statistics {
            let refreshes = statistics
                .SyncRefreshCount
                .wrapping_sub(previous.SyncRefreshCount);
            if refreshes > 2 {
                crate::viewport_bridge::record_viewport_perf("gpui_interval_over_2_refreshes", 1);
            }
            let qpc_ticks = statistics.SyncQPCTime - previous.SyncQPCTime;
            if refreshes > 0 && qpc_ticks > 0 {
                self.qpc_ticks_per_refresh = qpc_ticks as f64 / f64::from(refreshes);
            }
        }
        self.previous_frame_statistics = Some(statistics);
        ...
        let Some(submission) = self
            .present_history
            .iter()
            .find(|submission| submission.present_id == statistics.PresentCount)
        else {
            return;
        };
        let refresh_delta =
        ...
        let displayed_qpc =
            statistics.SyncQPCTime as f64 + refresh_delta as f64 * self.qpc_ticks_per_refresh;
        let latency_qpc = displayed_qpc - submission.submitted_qpc as f64;
        if latency_qpc < 0.0 {
            return;
        }
        let latency_ns = latency_qpc * 1_000_000_000.0 / self.qpc_frequency as f64;
        // Reject discontinuities caused by display-mode switches or stale
        // statistics. Genuine windowed-composition latency is far below this.
        if latency_ns <= 250_000_000.0 {
            crate::viewport_bridge::record_viewport_perf(
                "frame.present_submit_to_display",
                latency_ns.round() as u64,
            );
        }
    }
...
impl DirectXRenderPipelines {
    pub fn new(device: &ID3D11Device) -> Result<Self> {
        ...
        let composition_hole_pipeline = PipelineState::new(
            device,
            "composition_hole_pipeline",
            ShaderModule::Quad,
            4,
            create_composition_hole_blend_state(device)?,
        )?;
...
fn create_swap_chain_for_composition(
    ...
    let desc = DXGI_SWAP_CHAIN_DESC1 {
        ...
        Flags: swap_chain_flags(allow_tearing).0 as u32,
    };
...
fn present_tearing_supported(factory: &IDXGIFactory6) -> bool {
...
/// Erase premultiplied RGBA by the quad shader's clipped, antialiased coverage:
/// `destination *= 1 - source_alpha`. Later GPUI batches continue normally.
#[inline]
fn create_composition_hole_blend_state(device: &ID3D11Device) -> Result<ID3D11BlendState> {
    let mut desc = D3D11_BLEND_DESC::default();
    desc.RenderTarget[0].BlendEnable = true.into();
    desc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
    desc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
    ...
    unsafe {
        let mut state = None;
        device.CreateBlendState(&desc, Some(&mut state))?;
        Ok(state.context("composition-hole blend-state creation returned no state")?)
    }
}
...
const PRESENT_HISTORY_CAPACITY: usize = 64;
