...
impl WindowsWindowInner {
    pub(crate) fn handle_msg(
        ...
        let handled = match msg {
            ...
            WM_GPUI_FORCE_UPDATE_WINDOW => {
                // A zero wParam is the editor's latency-sensitive redraw poke:
                // bypass WM_PAINT priority without invoking GPUI's expensive
                // GPU-recovery full-refresh path. Existing recovery requests
                // carry the platform validation number and still force-render.
                let interaction_frame = wparam.0 == 0;
                let force_render = !interaction_frame;
                if !force_render {
                    crate::viewport_bridge::record_immediate_frame_message_received();
                }
                self.draw_window(handle, force_render, interaction_frame)
            }
    ...
    fn draw_window(
        ...
        crate::viewport_bridge::remember_interaction_window(handle);
