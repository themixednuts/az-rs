...
thread_local! {
    /// Window-thread registry used to resolve typed renderer capabilities from
    /// GPUI's stable window id. It deliberately stores weak references so a
    /// closed window cannot keep its renderer or composition tree alive.
    static WINDOWS_BY_ID: RefCell<HashMap<u64, Weak<WindowsWindowInner>>> =
...
impl WindowsWindow {
    pub(crate) fn new(
        ...
        WINDOWS_BY_ID.with(|windows| {
            windows
                .borrow_mut()
                .insert(this.handle.window_id().as_u64(), Rc::downgrade(&this));
        });
        Ok(Self(this))
    }
}
...
impl Drop for WindowsWindow {
    fn drop(&mut self) {
        WINDOWS_BY_ID.with(|windows| {
            windows
                .borrow_mut()
                .remove(&self.0.handle.window_id().as_u64());
        });
...
/// Return the typed viewport visual capability for one GPUI window.
/// The returned weak capability is invalidated automatically when the window
/// renderer is destroyed or rebuilt after device loss.
pub fn viewport_visual_slot(window_id: u64) -> Option<crate::ViewportVisualSlot> {
    WINDOWS_BY_ID.with(|windows| {
        let mut windows = windows.borrow_mut();
        let window = windows.get(&window_id)?.upgrade();
        ...
        window
            ...
            .renderer
            .borrow()
            .viewport_visual_slot(window_id)
    })
}
