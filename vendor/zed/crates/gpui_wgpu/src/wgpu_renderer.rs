...
use gpui::{
    AtlasTextureId, Background, BorderStyle, Bounds, CompositionHole, DevicePixels, Edges,
    GpuSpecs, MonochromeSprite, Path, Point, PolychromeSprite, PrimitiveBatch, Quad, ScaledPixels,
    Scene, Shadow, Size, SubpixelSprite, Underline, get_gamma_correction_ratios, rgba,
    transparent_black,
};
use log::warn;
...
struct WgpuPipelines {
    ...
    composition_holes: wgpu::RenderPipeline,
...
impl WgpuRenderer {
    ...
    fn create_pipelines(
        ...
        let composition_holes = create_pipeline(
            "composition_holes",
            "vs_quad",
            "fs_quad",
            &layouts.globals,
...

            {
                ...
                for batch in scene.batches() {
                    let ok = match batch {
                        ...
                        PrimitiveBatch::CompositionHoles(range) => self.draw_composition_holes(
                            &scene.composition_holes[range],
                            &mut instance_offset,
                            &mut pass,
                        ),
    ...
    fn draw_composition_holes(
        &self,
        holes: &[CompositionHole],
        instance_offset: &mut u64,
        pass: &mut wgpu::RenderPass<'_>,
    ) -> bool {
        let hole_quads = holes
            .iter()
            .map(|hole| Quad {
                order: hole.order,
            ...
            .collect::<Vec<_>>();
        let data = unsafe { Self::instance_bytes(&hole_quads) };
        self.draw_instances(
            data,
            hole_quads.len() as u32,
            &self.resources().pipelines.composition_holes,
            instance_offset,
            pass,
        )
    }
