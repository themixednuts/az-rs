...
pub use source_transform::{
    CollisionFilterColorSource, CollisionFilterSource, CollisionFiltersSource,
    CollisionFiltersSourceTransform, CollisionFiltersSourceTransformError,
    PhysicsMaterialSetSource, PhysicsMaterialSetSourceTransform,
    PhysicsMaterialSetSourceTransformError, PhysicsMaterialSource, collision_filters_source_path,
    is_legacy_collision_filters_source, is_legacy_physics_material_set_source,
    physics_material_set_source_path,
};

use az_asset_builder::{SourceFormat, source_schema_type};
use az_objectstream::value::ObjectStreamValueError;
use az_objectstream::visit::{ElementHeader, ElementVisitor, VisitFlow, parse_streaming_bytes};
use az_objectstream::{ObjectStreamError, types};
...
#[derive(SourceFormat)]
#[source(
    schema = "azoth.compat.azphysics.CollisionFiltersSource",
    ext = "collisionfilters.ron"
)]
pub struct CollisionFiltersSourceFormat;

#[derive(SourceFormat)]
#[source(
    schema = "azoth.compat.azphysics.PhysicsMaterialSetSource",
    ext = "physicsmaterialset.ron"
)]
pub struct PhysicsMaterialSetSourceFormat;

pub mod source_schemas {
    use super::{CollisionFiltersSourceFormat, PhysicsMaterialSetSourceFormat, source_schema_type};
    use az_asset_builder::SourceSchemaType;

    pub const COLLISION_FILTERS: SourceSchemaType =
        source_schema_type::<CollisionFiltersSourceFormat>();
    pub const PHYSICS_MATERIAL_SET: SourceSchemaType =
        source_schema_type::<PhysicsMaterialSetSourceFormat>();
}
...
/// Summarizes a `.collisionfilters` payload.
///
/// # Errors
///
...
impl CollisionFiltersTotals {
    pub const fn add_summary(&mut self, summary: CollisionFiltersSummary) {
        self.files += 1;
...
/// Summarizes one `.collisionfilters` payload and labels it with its display
/// path.
///
/// # Errors
///
/// Returns any error [`summarize_collision_filters_asset`] returns.
...
/// Reads and summarizes one `.collisionfilters` asset from disk.
///
/// # Errors
///
/// Returns [`CollisionFiltersInspectionError::Read`] when `path` cannot be
/// read, and [`CollisionFiltersInspectionError::Parse`] when its contents are
/// not a valid `CollisionFiltersAsset` `ObjectStream`.
...
/// Reads and aggregates every `.collisionfilters` asset in `paths`.
///
/// # Errors
///
/// Returns the first error [`inspect_collision_filters_path`] returns; the
/// walk stops at that path and the partial inspection is discarded.
...
impl MaterialSetAsset {
    ...
    /// Parse a `.physicsmaterialset` `ObjectStream` XML document.
    ///
    /// # Errors
    ///
    /// Returns [`MaterialSetParseError::Xml`] or
    /// [`MaterialSetParseError::Attribute`] for malformed XML,
    /// [`MaterialSetParseError::UnexpectedText`] for text inside the
    /// envelope, [`MaterialSetParseError::MissingObjectStream`],
    /// [`MaterialSetParseError::NestedObjectStream`], or
    /// [`MaterialSetParseError::UnsupportedObjectStreamVersion`] for a bad
    /// envelope, [`MaterialSetParseError::UnexpectedClass`],
    /// [`MaterialSetParseError::UnexpectedType`], or
    /// [`MaterialSetParseError::UnexpectedField`] for a class that is not the
    /// reflected `MaterialSetAsset` layout, and
    /// [`MaterialSetParseError::MissingField`],
    /// [`MaterialSetParseError::DuplicateField`],
    /// [`MaterialSetParseError::InvalidNumber`], or
    /// [`MaterialSetParseError::InvalidBool`] for material properties that are
    /// missing, repeated, or unparsable.
...
/// Summarizes a `.physicsmaterialset` payload.
///
/// # Errors
///
...
impl MaterialSetTotals {
    pub const fn add_summary(&mut self, summary: MaterialSetSummary) {
        self.files += 1;
...
/// Summarizes one `.physicsmaterialset` payload and labels it with its display
/// path.
///
/// # Errors
///
/// Returns any error [`summarize_material_set_asset`] returns.
...
/// Reads and summarizes one `.physicsmaterialset` asset from disk.
///
/// # Errors
///
/// Returns [`MaterialSetInspectionError::Read`] when `path` cannot be read,
/// and [`MaterialSetInspectionError::Parse`] when its contents are not a valid
/// `MaterialSetAsset` `ObjectStream` XML document.
...
/// Reads and aggregates every `.physicsmaterialset` asset in `paths`.
///
/// # Errors
///
/// Returns the first error [`inspect_material_set_path`] returns; the walk
...
#[must_use]
pub const fn is_collision_filters_extension(extension: &str) -> bool {
...
#[must_use]
pub const fn is_physics_material_set_extension(extension: &str) -> bool {
...
impl MaterialSetXmlParser {
    ...
    fn end_class(&mut self) -> Result<(), MaterialSetParseError> {
            ...
            .ok_or(MaterialSetParseError::EndWithoutClass)?
        {
            ...
            // `MaterialSetAsset` and the `Materials` list carry no state of
            // their own: their children are committed as each child closes.
            MaterialScope::Asset | MaterialScope::Materials => Ok(()),
    ...
    fn read_filter_field(
        ...
        match header.name_crc {
            ...
            Some(INHERITS_FILTERS_FIELD_CRC) => {
                expect_type(header, STRING_VECTOR_TYPE_ID)?;
                let current = self.current_filter_mut(header)?;
                if current.seen.contains(FilterListsSeen::INHERITS_FILTERS) {
                    return Err(CollisionFiltersParseError::DuplicateFilterField {
                        index,
                        field: "InheritsFilters",
                    });
                }
                current.seen.insert(FilterListsSeen::INHERITS_FILTERS);
                self.start_string_list(ActiveStringListTarget::FilterInherits, self.depth)?;
            }
            Some(IS_CATEGORIES_FIELD_CRC) => {
                expect_type(header, STRING_VECTOR_TYPE_ID)?;
                let current = self.current_filter_mut(header)?;
                if current.seen.contains(FilterListsSeen::IS_CATEGORIES) {
                    return Err(CollisionFiltersParseError::DuplicateFilterField {
                        index,
                        field: "IsCategories",
                    });
                }
                current.seen.insert(FilterListsSeen::IS_CATEGORIES);
            ...
            Some(COLLIDE_WITH_CATEGORIES_FIELD_CRC) => {
                ...
                if current
                    .seen
                    .contains(FilterListsSeen::COLLIDE_WITH_CATEGORIES)
                {
                    return Err(CollisionFiltersParseError::DuplicateFilterField {
                        index,
                ...
                current
                    .seen
                    .insert(FilterListsSeen::COLLIDE_WITH_CATEGORIES);
            ...
            Some(FILTER_TAGS_FIELD_CRC) => {
                ...
                let current = self.current_filter_mut(header)?;
                if current.seen.contains(FilterListsSeen::FILTER_TAGS) {
                    return Err(CollisionFiltersParseError::DuplicateFilterField {
                        index,
                        field: "FilterTags",
                    });
                }
                current.seen.insert(FilterListsSeen::FILTER_TAGS);
...
/// Which of an `EditableCollisionFilter`'s list fields the reader has opened.
///
/// An empty list is legal, so presence cannot be inferred from the collected
/// values; this records it as a bit per field instead of a bool per field.
#[derive(Debug, Default, Clone, Copy, PartialEq, Eq)]
struct FilterListsSeen(u8);

impl FilterListsSeen {
    const INHERITS_FILTERS: Self = Self(1 << 0);
    const IS_CATEGORIES: Self = Self(1 << 1);
    const COLLIDE_WITH_CATEGORIES: Self = Self(1 << 2);
    const FILTER_TAGS: Self = Self(1 << 3);

    const fn contains(self, field: Self) -> bool {
        self.0 & field.0 == field.0
    }

    const fn insert(&mut self, field: Self) {
        self.0 |= field.0;
    }
}
...
impl PartialEditableCollisionFilter {
    fn finish(self, index: usize) -> Result<EditableCollisionFilter, CollisionFiltersParseError> {
        if !self.seen.contains(FilterListsSeen::INHERITS_FILTERS) {
            return Err(CollisionFiltersParseError::MissingFilterField {
                index,
                field: "InheritsFilters",
            });
        }
        if !self.seen.contains(FilterListsSeen::IS_CATEGORIES) {
            return Err(CollisionFiltersParseError::MissingFilterField {
                index,
                field: "IsCategories",
            });
        }
        if !self.seen.contains(FilterListsSeen::COLLIDE_WITH_CATEGORIES) {
            return Err(CollisionFiltersParseError::MissingFilterField {
                index,
                field: "CollideWithCategories",
            });
        }
        if !self.seen.contains(FilterListsSeen::FILTER_TAGS) {
            return Err(CollisionFiltersParseError::MissingFilterField {
                index,
                field: "FilterTags",
            });
...
mod tests {
    ...
    fn parses_material_set() {
        ...
        assert_exact(material_set.default_material.friction, 0.5);
        assert_exact(material_set.default_material.restitution, 0.0);
        ...
        assert_exact(material.friction, 0.6);
        assert_exact(material.restitution, 0.1);
    ...
    fn collision_filters_fixture(include_filter_tags: bool) -> Vec<u8> {
        ...
        stream.elements = vec![
            Element::new(COLLISION_FILTERS_ASSET_TYPE_ID)
                ...
                .with_children(vec![
                    ...
                    named_element(
                        ...
                        vec![
                            Element::new(COLLISION_FILTER_COLOR_TYPE_ID)
                                .with_wire_flags(ST_BINARYFLAG_ELEMENT_HEADER)
                                .with_children(vec![
  ...
  <Class name="MaterialSetAsset" type="{9E366D8C-33BB-4825-9A1F-FA3ADBE11D0F}">
    ...
    fn filter_element(filter_tags: Option<Element>) -> Element {
        ...
        Element::new(EDITABLE_COLLISION_FILTER_TYPE_ID)
            .with_wire_flags(ST_BINARYFLAG_ELEMENT_HEADER)
    ...
    /// Compares a decoded colour channel-by-channel, bit-exactly.
    ///
    /// The fixture writes the same big-endian `f32` pattern the reader decodes,
    /// so any difference is a decode bug rather than accumulated error.
    #[track_caller]
