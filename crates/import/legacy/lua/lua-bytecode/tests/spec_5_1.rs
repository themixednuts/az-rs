...
/// Cases for `opcode_runtime_equivalence_matrix_exercises_all_lua_51_opcodes`, hoisted so the test body stays readable.
const OPCODE_MATRIX_CASES: &[OpcodeCase] = &[
    OpcodeCase {
...
fn opcode_runtime_equivalence_matrix_exercises_all_lua_51_opcodes() {
    ...
    for case in OPCODE_MATRIX_CASES {
        let Some(bytecode) = compile_source_bytes(case.name, case.source, false) else {
            return;
        };
        let disassembly = az_lua_bytecode::disassemble(&bytecode).expect("disassemble case");
        let ssa = az_lua_bytecode::ssa_dump(&bytecode).expect("SSA dump case");
        assert!(
            !ssa.trim().is_empty(),
...
fn dominating_nil_declaration_is_not_replanned_as_a_multi_local_initializer() {
    ...
    let source = az_lua_bytecode::decompile(&bytecode).expect("decompile separate nil declaration");
    assert_eq!(
        source,
        concat!(
            "spec_global = 5\n",
...
fn chunk_reader_handles_lua_51_layout_variants_and_rejects_bad_header_fields() {
    ...
    // 2^60 is a power of two, so it is exactly representable as an f64.
    assert_number_constant(&exact_integral64, 2f64.powi(60));
    ...
    assert!(
        az_lua_bytecode::parse_chunk(&inexact_integral64).is_err(),
    ...
    assert!(
        az_lua_bytecode::parse_chunk(&bad_instruction_size).is_err(),
...
fn number_constants(bytecode: &[u8]) -> Vec<f64> {
    let chunk = az_lua_bytecode::parse_chunk(bytecode).expect("parse bytecode");
