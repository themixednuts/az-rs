...
const CUSTOM_OPCODE_TABLE: &str = r"
VERSION 51
OPBITS 6
ABITS 8
BBITS 9
...
#[test]
fn standard_lua_51_table_decodes_synthetic_chunk_without_unknown_opcodes() {
...
#[test]
fn disassembles_synthetic_branch_stably() {
    let chunk = parse_chunk(SYNTHETIC_BRANCH).expect("synthetic chunk parses");
    let table = lua51_table();
    ...
    let proto_output = disassemble_proto(&chunk.root, &table);
    let code_lines = instruction_lines(&proto_output);

    assert_eq!(code_lines.len(), chunk.root.code.len());
    assert!(code_lines[0].contains("[   1]"));
    assert!(code_lines[0].contains("LOADK"));
    assert!(code_lines.iter().any(|line| line.contains("JMP")));
    assert!(
        code_lines
            .last()
...
fn parses_custom_opcode_table_text() {
    let table = OpcodeTable::from_custom_text(CUSTOM_OPCODE_TABLE).expect("custom table parses");
