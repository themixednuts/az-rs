...
use support::{compile_source_bytes, external_fixture, run_equivalence};
...
/// Cases for `reconstructs_short_circuit_values_for_non_local_consumers`, hoisted so the test body stays readable.
const NON_LOCAL_CONSUMER_CASES: &[(&str, &str)] = &[
    (
...
fn reconstructs_short_circuit_values_for_non_local_consumers() {
    for (name, source) in NON_LOCAL_CONSUMER_CASES {
        let _ = run_equivalence(name, source);
    }
}
...
",
    ...
    let decompiled = az_lua_bytecode::decompile(&bytecode).expect("minimal repro decompiles");
...
",
    ...
    let decompiled = az_lua_bytecode::decompile(&bytecode).expect("R2 shape repro decompiles");
...
"#,
    ...
    let decompiled = az_lua_bytecode::decompile(&bytecode).expect("R3 boolean residual decompiles");
...
"#,
    ...
    let decompiled =
        az_lua_bytecode::decompile(&bytecode).expect("guarded value component decompiles");
...
fn convert_seconds_short_circuit_reconstruction_is_faithful() {
    let Some(source) = external_fixture("lyshineui/_common/timehelperfunctions.lua") else {
        return;
    };
    let Some(bytecode) = compile_file_bytes("r2_timehelper", &source, false) else {
        return;
    };
    let decompiled = az_lua_bytecode::decompile(&bytecode).expect("timehelperfunctions decompiles");
