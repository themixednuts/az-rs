...
#[test]
fn synthetic_branch_reparses_and_has_no_undefined_synthetic_names() {
    let source = az_lua_bytecode::decompile(SYNTHETIC_BRANCH).expect("synthetic chunk decompiles");
    full_moon::parse(&source).unwrap_or_else(|errors| {
        panic!("synthetic chunk emitted source did not parse:\n{source}\n{errors:#?}")
    });
...
fn lex_tokens(source: &str) -> Vec<String> {
    ...
    while index < bytes.len() {
            ...
            || bytes[index..].starts_with(b">=")
        {
            ...
            tokens.push(source[index..=index].to_string());
            index += 1;
        }
    }
    tokens
