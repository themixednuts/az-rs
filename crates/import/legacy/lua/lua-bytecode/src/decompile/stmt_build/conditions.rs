use super::{
    ConditionChain, Expr, LuaError, NodeEmission, SsaNode, SsaRef, StatementBuilder, Stmt,
    ValuePlan, assign_one, is_inlineable_def, normalize,
};
...
impl StatementBuilder<'_> {
    ...
    pub(crate) fn phi_assignment(
        ...
        let mut value = self.exprs.expr_for_ref(operand, pc)?;
        if value == Expr::Name(name.clone()) {
            let omitted_def = self
                .analysis
                .def_site(operand)
                .filter(|id| matches!(self.plan.node_emission(*id), NodeEmission::Omitted))
                .and_then(|id| self.analysis.node(self.function, id))
                .filter(|definition| is_inlineable_def(&definition.op))
                .cloned();
            if let Some(definition) = omitted_def {
                value = self.exprs.node_expr(&definition)?;
            }
        }
        if value == Expr::Name(name.clone()) {
            return Ok(None);
