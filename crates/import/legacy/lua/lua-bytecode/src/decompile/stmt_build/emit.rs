use super::{
    BString, Constant, Expr, FuncBody, LFIELDS_PER_FLUSH, LuaError, Name, NodeId, SsaNode, SsaOp,
    SsaRef, StatementBuilder, Stmt, TableField, assign_one, ast, global_expr_from_name, index_expr,
    is_stable_assignment_target, multi,
};
...
impl StatementBuilder<'_> {
    pub(super) fn emit_node(
        ...
        match &node.op {
            // Emitted nothing: these are consumed by control-flow
            // reconstruction rather than by statement emission.
            SsaOp::Nop
            | SsaOp::Jump { .. }
            | SsaOp::Close { .. }
            | SsaOp::Branch { .. }
    ...
    pub(super) fn emit_open_setlist_fallback(
        ...
        let first_index = (i64::from(batch) - 1) * LFIELDS_PER_FLUSH + 1;
        let pack_name = Name::from(format!("__az_lua_pack_{}", node.pc));
        let values_name = Name::from(format!("__az_lua_values_{}", node.pc));
        let index_name = Name::from(format!("__az_lua_index_{}", node.pc));
