...
pub(super) fn collect_forced_loop_values(
    ...
    match region {
        ...
        Region::If(region) => {
            force_phi_values(&region.phis, |_| false, forced);
        ...
        Region::While(region) => {
            ...
            force_exit_phi_values(function, region.exit, |_| false, forced);
        ...
        Region::NumericFor(region) => {
            ...
            force_exit_phi_values(
                function,
                Some(region.info.exit),
                |reg| reg >= base && reg <= base.saturating_add(3),
                forced,
            );
            collect_forced_loop_values(&region.body, function, forced);
...
fn force_exit_phi_values(
    ...
    let phis = conditionals::phi_sources(function, exit);
    force_phi_values(&phis, exclude, forced);
}
