...
/// One step of `ChainParser::parse_group`.
enum GroupStep {
    /// The group ends here, carrying this pending `or` continuation.
    Done(Option<usize>),
    /// Keep walking the chain from this block.
    Continue(usize),
}
...
impl ChainParser<'_> {
    ...
    fn parse_group(&mut self, start: usize) -> Option<(Vec<ValueTerm>, Option<usize>)> {
        ...
        for _ in 0..16 {
            ...
            let targets = (
                self.follow_value_target(info.true_block),
                self.follow_value_target(info.false_block),
            );
            let sources = (
                self.source_operand(targets.0),
                self.source_operand(targets.1),
            );
            let step = if let (Some(true_operand), Some(false_operand)) = sources {
                self.step_paired_sources(
                    &info,
                    rel,
                    targets,
                ...
                self.step_single_source(&info, targets, sources, &mut terms, &mut next_or)?
            };
            match step {
                GroupStep::Done(next) => return Some((terms, next)),
                GroupStep::Continue(block) => current = block,
            }
        }
    ...
    /// Step the chain across a branch whose successors both carry a phi source.
    ///
    /// This is where `a and b`, `a or b`, and the boolean-materialising
    /// `x = cond` shapes are told apart.
    fn step_paired_sources(
        &mut self,
        info: &conditionals::BranchInfo,
        rel: RelOp,
        targets: (usize, usize),
        operands: (SsaRef, SsaRef),
        terms: &mut Vec<ValueTerm>,
        next_or: Option<usize>,
    ) -> Option<GroupStep> {
        let (true_target, false_target) = targets;
        let (true_operand, false_operand) = operands;
        ...
        ) && true_value != false_value
        {
            ...
            return Some(GroupStep::Done(next_or));
        }
        if rel == RelOp::Test && self.source_bool_value(false_target, false_operand) == Some(false)
        {
            self.mark_source_used(false_target, false_operand);
        ...
        if rel == RelOp::Test && self.source_bool_value(true_target, true_operand) == Some(false) {
            self.mark_source_used(true_target, true_operand);
            terms.push(ValueTerm::Condition {
                branch: info.node,
                inverted: true,
        ...
        if source_matches_branch_value(self.function, info.node, true_operand) {
            ...
            terms.push(self.source_term(true_target)?);
            return Some(GroupStep::Done(Some(false_target)));
        }
        if source_matches_branch_value(self.function, info.node, false_operand) {
            ...
            terms.push(self.source_term(false_target)?);
            return Some(GroupStep::Done(Some(true_target)));
        }
        if self
    ...
    /// Step the chain across a branch where at most one successor carries a
    /// phi source.
    fn step_single_source(
        &mut self,
        info: &conditionals::BranchInfo,
        targets: (usize, usize),
        sources: (Option<SsaRef>, Option<SsaRef>),
        terms: &mut Vec<ValueTerm>,
        next_or: &mut Option<usize>,
    ) -> Option<GroupStep> {
        let (true_target, false_target) = targets;
        match sources {
            (Some(true_operand), None) if !terms.is_empty() => {
                ...
                Some(GroupStep::Done(Some(false_target)))
            }
                ...
                if source_matches_branch_value(self.function, info.node, true_operand) =>
            {
                if branch_test_invert(self.function, info.node)? {
                    self.mark_source_used(true_target, true_operand);
                    terms.push(positive_test_condition(self.function, info.node)?);
                    return Some(GroupStep::Continue(false_target));
                }
                terms.push(self.source_term(true_target)?);
                Some(GroupStep::Done(Some(false_target)))
            }
            // Unrecognised. `(Some, Some)` never arrives here: `parse_group`
            // routes the paired case to `step_paired_sources`.
            (Some(_), _) => None,
            (None, Some(false_operand)) if !terms.is_empty() => {
                ...
                if !branch_test_invert(self.function, info.node)? {
                    let _ = false_operand;
                    if !self.set_next_or(next_or, false_target) {
                        return None;
                    }
                    terms.push(positive_test_condition(self.function, info.node)?);
                    return Some(GroupStep::Continue(true_target));
                }
                let _ = false_operand;
                terms.push(self.source_term(false_target)?);
                Some(GroupStep::Done(Some(true_target)))
            }
            // Both remaining shapes fall through to the same step: record
            // the pending `or` continuation and keep walking the true side.
            (None, Some(_) | None) => {
                if !self.set_next_or(next_or, false_target) {
                    return None;
                }
                ...
                Some(GroupStep::Continue(true_target))
            }
        }
    }

    const fn set_next_or(&self, next_or: &mut Option<usize>, target: usize) -> bool {
        if target == self.merge {
            return false;
        }
        if let Some(current) = *next_or {
            current == target
        } else {
            *next_or = Some(target);
            true
