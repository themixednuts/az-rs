...
pub struct TableConstructorPlan {
    table: SsaRef,
    end: NodeId,
    setlists: Vec<NodeId>,
    keyed: Vec<NodeId>,
...
impl TableConstructorPlan {
    ...
    #[must_use]
    pub(crate) const fn mutation_count(&self) -> usize {
...
pub fn is_matching_setlist(node: &SsaNode, table: SsaRef, table_reg: u16) -> bool {
    matches!(
        &node.op,
        SsaOp::SetList {
            table: setlist_table,
...
pub fn is_matching_settable(node: &SsaNode, table: SsaRef, table_reg: u16) -> bool {
    matches!(
        &node.op,
        SsaOp::SetTable {
