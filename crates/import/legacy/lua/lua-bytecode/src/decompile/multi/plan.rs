...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct TableConstructorEmission {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LocalDeclarationPlan {
    pub reference: SsaRef,
    pub name: Name,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LocalValuePlan {
    pub node: NodeId,
    pub refs: Vec<SsaRef>,
    pub names: Vec<Name>,
}
...
const fn is_constructor_mutation(node: &SsaNode) -> bool {
    matches!(&node.op, SsaOp::SetTable { .. } | SsaOp::SetList { .. })
}

const fn is_direct_value_node(node: &SsaNode) -> bool {
    match &node.op {
        SsaOp::LoadK { .. }
        | SsaOp::LoadLiteral { .. }
        | SsaOp::LoadBool { .. }
