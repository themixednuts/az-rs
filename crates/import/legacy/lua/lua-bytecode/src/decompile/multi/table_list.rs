...
pub enum ConstructorValueMode {
    Normal,
    FixedLast,
}

pub fn fields_from_nodes<'a, S, K, F>(
    setlists: S,
    keyed: K,
    expr_for_ref: &mut F,
) -> Result<Vec<TableField>, LuaError>
...
fn key_may_overlap_list(key: &Expr, list_count: usize) -> bool {
    ...
    match key {
        ...
        Expr::Number(value) if value.fract() == 0.0 && *value >= 1.0 => {
            // A `Vec` length is far below 2^53, so this converts exactly;
            // std has no checked integer-to-float conversion.
            #[allow(clippy::cast_precision_loss)]
            let count = list_count as f64;
            *value <= count
        }
