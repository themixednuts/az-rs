...
use crate::{
    ...
    ir::{SsaFunction, SsaOp, SsaRef, UpvalueCapture},
};

pub use super::identifier::is_valid_identifier;

const LUA_VARARG_NEEDS_ARG_TABLE: u8 = 4;
...
impl<'a> NameResolver<'a> {
    ...
    pub fn new(proto: &'a Proto, function: &SsaFunction) -> Self {
        Self::with_overrides(proto, function, FunctionId::root(), Vec::new(), &[])
    }
    ...
    /// # Panics
    ///
    /// Panics if `reference` is the legacy vararg table but the prototype was
    /// not compiled with `NEEDSARG`, so no `arg` name exists to return.
    #[must_use]
    pub fn name_for_ref(&self, reference: SsaRef, pc: i32) -> Name {
        if self.is_legacy_vararg_table_ref(reference) {
            return self
                .legacy_vararg_table_name()
                .expect("legacy vararg reference requires NEEDSARG");
        }
    ...
    pub fn synthetic_value_name(&self, reference: SsaRef) -> Name {
        ...
        // Not `map_or`: the `Some` arm consumes `name`, so a closure-based
        // form would have to clone it on every call.
        match self.binding_id_for_ref(reference) {
            Some(binding) => name.with_binding(binding),
            None => name,
        }
    }
    ...
    /// Lua 5.1's compatibility vararg table is an implicit local named `arg`.
    /// The `NEEDSARG` prototype flag reserves the register immediately after
    /// the fixed parameters for this table.
    #[must_use]
    pub fn legacy_vararg_table_name(&self) -> Option<Name> {
        (self.proto.is_vararg & LUA_VARARG_NEEDS_ARG_TABLE != 0)
            .then(|| Name::new("arg").with_binding(BindingId::legacy_vararg(&self.function_id)))
    }

    fn is_legacy_vararg_table_ref(&self, reference: SsaRef) -> bool {
        self.proto.is_vararg & LUA_VARARG_NEEDS_ARG_TABLE != 0
            && reference.reg_index() == Some(u16::from(self.proto.num_params))
    }
    ...
    pub fn implicit_entry_declarations(&self) -> Vec<Name> {
        self.local_bindings
            .iter()
            .filter(|binding| {
                binding.index >= usize::from(self.proto.num_params)
                    && binding.start_pc == 0
                    && !(self.proto.is_vararg & LUA_VARARG_NEEDS_ARG_TABLE != 0
                        && binding.reg == u16::from(self.proto.num_params)
                        && binding.name.as_bytes() == b"arg")
            })
            .map(|binding| {
                binding
                    .name
    ...
    /// Return whether an SSA value without an instruction definition is
    /// nevertheless introduced by the function entry contract.
    #[must_use]
    pub fn is_implicit_entry_ref(&self, reference: SsaRef) -> bool {
        let SsaRef::Reg { reg, ver: 0 } = reference else {
            return false;
        };
        if reg < u16::from(self.proto.num_params) || self.is_legacy_vararg_table_ref(reference) {
            return true;
        }
        let Some(identity) = self.binding_id_for_ref(reference) else {
            return false;
        };
        self.local_bindings
            .iter()
            .enumerate()
            .any(|(index, binding)| {
                binding.reg == reg && binding.start_pc == 0 && self.debug_binding(index) == identity
            })
    }
...
impl<'a> NamingPass<'a> {
    ...
    fn run(mut self) -> NamingResult {
        ...
        self.union_self_recursive_closures();
        self.union_parameter_values();
        self.union_legacy_vararg_table_values();
    ...
    /// A Lua 5.1 local recursive function closes over the destination register
    /// before `CLOSURE` writes the function value into that same register.
    /// SSA necessarily gives the captured cell and the written value different
    /// versions, but they remain one source-level binding.
    fn union_self_recursive_closures(&mut self) {
        let pairs = self
            .function
            .blocks
            .iter()
            ...
            .flat_map(|node| {
                ...
                let Some(destination_reg) = node.dest.reg_index() else {
                    return Vec::new();
                };
                upvalues
                    .iter()
                    .filter_map(|capture| match capture {
                        UpvalueCapture::ParentLocal(captured)
                            if captured.reg_index() == Some(destination_reg) =>
                        {
                            Some((node.dest, *captured))
                        }
                        _ => None,
                    })
                    .collect::<Vec<_>>()
        ...
        for (destination, captured) in pairs {
            if let (Some(destination), Some(captured)) =
                (self.ensure_ref(destination), self.ensure_ref(captured))
            {
                self.union(destination, captured);
            }
        }
    }

    /// Function parameters remain one lexical local for the entire function,
    /// even though assignments create new SSA versions of their registers.
    /// Lua's register allocator cannot reuse a parameter slot for a distinct
    /// local while the parameter is still in lexical scope.
    fn union_parameter_values(&mut self) {
        for reg in 0..u16::from(self.proto.num_params) {
            let ids = self
                .values
                .iter()
                .enumerate()
                .filter_map(|(id, slot)| (slot.value.reg == reg).then_some(id))
                .collect::<Vec<_>>();
    ...
    fn union_legacy_vararg_table_values(&mut self) {
        if !self.has_legacy_vararg_table() {
            return;
        }
        let ids = self
            .values
            ...
            .enumerate()
            .filter_map(|(id, slot)| {
                (slot.value.reg == u16::from(self.proto.num_params)).then_some(id)
            })
            .collect::<Vec<_>>();
        let Some((&first, rest)) = ids.split_first() else {
            return;
    ...
    fn assign_names(mut self) -> NamingResult {
        ...
        for id in 0..self.values.len() {
            ...
            if self.has_legacy_vararg_table()
                && slot.value.reg == u16::from(self.proto.num_params)
                && slot.value.ver == 0
            {
                info.has_legacy_vararg_table = true;
            }
        }
        ...
        for group in groups {
            let (name, binding) = if group.has_legacy_vararg_table {
                (Name::new("arg"), BindingId::legacy_vararg(self.function_id))
            } else if let Some((index, name)) = group.debug_binding {
                ...
                let name = if let Some(name) = anonymous_locals.get(&group.first_reg) {
                    ...
                    let base = AnonymousNameRole::Local.base(group.first_reg);
    ...
    const fn has_legacy_vararg_table(&self) -> bool {
        self.proto.is_vararg & LUA_VARARG_NEEDS_ARG_TABLE != 0
    }
}
...
const fn binding_in_scope(binding: &LocalBinding, pc: i32) -> bool {
...
fn unique_name(base: &str, used: &mut BTreeSet<String>) -> Name {
...
fn is_synthetic_identifier(bytes: &[u8]) -> bool {
    has_prefixed_number(bytes, b"a", true)
        ...
        || has_prefixed_number(bytes, b"__az_lua_pack_", false)
        || has_prefixed_number(bytes, b"__az_lua_values_", false)
        || has_prefixed_number(bytes, b"__az_lua_index_", false)
}
