use super::{
    Expr, ExprBuilder, LuaError, NodeId, SsaNode, SsaOp, SsaRef, ValueId,
    constructor_mutation_table, direct_eval_order_refs, is_inlineable_def, is_pure_def,
};
...
impl ExprBuilder<'_> {
    ...
    pub(super) fn reg_expr(&mut self, reference: SsaRef, use_pc: i32) -> Result<Expr, LuaError> {
        ...
        let Some(node_id) = self.analysis.def_site(reference) else {
            let name = self.names.name_for_ref(reference, use_pc);
            return Ok(if self.names.is_implicit_entry_ref(reference) {
                Expr::Name(name)
            } else {
                Expr::Nil
            });
        ...
        if let SsaOp::Branch {
            rel: crate::ir::RelOp::TestSet,
            a,
            ..
        } = &node.op
            && node.dest == reference
        {
            return self.expr_for_ref(*a, use_pc);
        }
