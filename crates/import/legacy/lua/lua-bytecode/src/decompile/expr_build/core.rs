use super::{
    BString, Constant, Expr, ExprBuilder, LuaError, Name, SsaNode, SsaOp, SsaRef, ValueId, ast,
    closure, global_expr_from_name, index_expr, ir, map_bin_op, map_un_op,
};

impl ExprBuilder<'_> {
    /// Convert an SSA reference to the expression it denotes at `use_pc`.
    ///
    /// # Errors
    ///
    /// Returns [`LuaError::Malformed`] when a constant index is out of range,
    /// or [`LuaError::Unsupported`] when a register has no reachable defining
    /// node at `use_pc`.
    ...
    /// # Errors
    ///
    /// Returns [`LuaError::Unsupported`] when `node` has no expression form —
    /// a statement-only op, or a call reached outside a value context — and
    /// any error [`Self::expr_for_ref`] returns for its operands.
