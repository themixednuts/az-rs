use super::{
    Expr, ExprBuilder, LuaError, NodeId, SsaNode, SsaOp, SsaRef, ValuePlan, ValuePlanKind,
    ValueTerm, ast, conditionals, ir, normalize,
};
...
impl ExprBuilder<'_> {
    ...
    /// An `and`-chain with an `or` fallback.
    fn expr_for_chain_plan(
        &mut self,
        terms: &[ValueTerm],
        fallback: ValueTerm,
        pc: i32,
        ...
        let Some(first) = terms.next() else {
            return self.expr_for_value_term(fallback, pc);
        };
        let mut selected = self.expr_for_value_term(first, pc)?;
        for term in terms {
            selected = Expr::Binary {
                op: ast::BinOp::And,
                lhs: Box::new(selected),
                rhs: Box::new(self.expr_for_value_term(term, pc)?),
            };
        }
        Ok(Expr::Binary {
            op: ast::BinOp::Or,
            lhs: Box::new(selected),
            rhs: Box::new(self.expr_for_value_term(fallback, pc)?),
        })
    }

    /// Groups of `and`-chains joined by `or`.
    fn expr_for_and_or_chain(
        &mut self,
        groups: &[Vec<ValueTerm>],
        pc: i32,
    ) -> Result<Expr, LuaError> {
        ...
        for group in groups {
            ...
            let mut expr = self.expr_for_value_term(first, pc)?;
            for term in terms {
                expr = Expr::Binary {
                    op: ast::BinOp::And,
                    lhs: Box::new(expr),
    ...
    fn expr_for_value_plan_kind(
        ...
        let expr = match kind {
            ...
            ValuePlanKind::Chain { terms, fallback } => {
                self.expr_for_chain_plan(terms, *fallback, plan.pc)?
            }
            ValuePlanKind::AndOrChain { groups } => self.expr_for_and_or_chain(groups, plan.pc)?,
