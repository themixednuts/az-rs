...
pub fn is_elseif_candidate(
    ...
    let is_condition_node = |id: NodeId| {
        id.block == block
            && (id.node == branch.node.node
                || condition_nodes.get(id.node).copied().unwrap_or(false))
    };
    let condition_is_self_contained = block_ref
        .nodes
        .iter()
        .enumerate()
        .filter(|(index, _)| condition_nodes[*index])
        .all(|(index, _)| {
            analysis
                .defs_at(NodeId { block, node: index })
                .iter()
                .copied()
                .all(|reference| {
                    analysis.facts(reference).upvalue_captures == 0
                        && analysis
                            ...
                            .iter()
                            .copied()
                            .all(is_condition_node)
                })
        });

    condition_is_self_contained
...
fn all_paths_reach_candidate_or_terminate(
    ...
    if is_terminal(function, block) {
        return !can_reach(function, candidate, block);
    }
    match state[block] {
...
const fn is_return_node(node: &SsaNode) -> bool {
