...
impl Region {
    ...
    pub fn owns_value(&self, value: SsaRef) -> bool {
        match self {
            Self::Sequence(regions) => regions.iter().any(|region| region.owns_value(value)),
            Self::Value(region) => region.plan.dest == value,
            Self::If(region) => {
            ...
            Self::While(region) => region.body.owns_value(value),
            Self::Repeat(region) => region.body.owns_value(value),
            Self::NumericFor(region) => region.body.owns_value(value),
            Self::GenericFor(region) => region.body.owns_value(value),
            Self::Linear(_) | Self::Break => false,
        }
    }
    ...
    fn collect_blocks(&self, out: &mut BTreeSet<usize>) {
        match self {
            ...
            Self::Repeat(region) => region.body.collect_blocks(out),
            Self::NumericFor(region) => {
                out.insert(region.info.prep);
                out.insert(region.info.loop_block);
                region.body.collect_blocks(out);
            }
            Self::GenericFor(region) => {
