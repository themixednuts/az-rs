...
impl ReconstructionPlan {
    ...
    pub fn build(
        ...
        if let Some(legacy_arg) = names.legacy_vararg_table_name() {
            plan.entry_declarations.insert(0, legacy_arg);
        }
...
const fn is_fixed_result_statement(node: &SsaNode) -> bool {
    matches!(
        &node.op,
...
fn retain_declaration_anchors(
    ...
    let live_bindings = decisions
        ...
        .filter(super::ast::BindingId::is_debug_local)
        .collect::<HashSet<_>>();
...
fn classify_value(
    ...
    if forced.contains(&reference)
        ...
        || closure::captures_destination(node, reference)
    ...
    if booleans.value_for_phi(reference).is_some() {
        let has_debug_name = names.has_debug_binding(reference);
        return if has_debug_name || analysis.real_use_count(reference) > 1 {
            ValueDisposition::Materialize
        } else {
