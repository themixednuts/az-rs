...
pub(crate) fn try_local_function(
    ...
    let binding = builder.binding_for_def(reg, pc);
    if binding.is_none() && !captures_destination(node, node.dest) {
        return Ok(None);
    }
    ...
    let name = binding.map_or_else(
        || builder.name_for_ref(node.dest, pc),
...
/// Whether a closure captures the local cell in the register it replaces.
///
/// Lua 5.1 uses this bytecode shape for a recursive local function: the
/// closure captures the register's existing cell, then `CLOSURE` stores the
/// function into that cell. Source emission must therefore use
/// `local function name(...)`, which puts the binding in scope inside its body.
pub(crate) fn captures_destination(node: &SsaNode, reference: ir::SsaRef) -> bool {
    let Some(destination_reg) = reference.reg_index() else {
        return false;
    };
    matches!(
        &node.op,
...
const fn is_vararg(proto: &Proto) -> bool {
    proto.is_vararg & 2 != 0
}
