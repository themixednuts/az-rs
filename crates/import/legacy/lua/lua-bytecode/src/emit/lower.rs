...
pub fn lower_block(block: &ast::Block) -> full_moon::ast::Block {
    ...
    for (index, stmt) in block.0.iter().enumerate() {
        let is_last = index + 1 == block.0.len();
        match stmt {
            Stmt::Return(values) if is_last => {
                last_stmt = Some(builder::return_stmt(lower_exprs(values)));
            }
            Stmt::Break if is_last => {
                last_stmt = Some(builder::break_stmt());
            }
            Stmt::Return(values) => stmts.push(builder::do_stmt(builder::block(
                Vec::new(),
                Some(builder::return_stmt(lower_exprs(values))),
            ))),
            Stmt::Break => stmts.push(builder::do_stmt(builder::block(
                Vec::new(),
                Some(builder::break_stmt()),
            ))),
...
fn lower_expr(expr: &Expr, context: ExprContext) -> Expression {
    match expr {
        ...
        Expr::Global(_) | Expr::Index { .. } | Expr::Field { .. } => {
...
fn lower_var(expr: &Expr) -> Var {
    match expr {
        ...
        Expr::Global(_) | Expr::Index { .. } | Expr::Field { .. } | Expr::Call { .. } => {
            let (prefix, suffixes) = lower_prefix(expr);
...
impl ExprContext {
    const fn binary_child(op: BinOp, side: Side) -> Self {
        Self {
            parent: Some(ParentExpr::Binary { op, side }),
        }
    }

    const fn unary_child(op: UnOp) -> Self {
        Self {
...
impl ExprShape {
    const fn precedence(self) -> u8 {
        match self {
            Self::Binary(op) => precedence(op),
            Self::Unary(_) => PREC_UNARY,
        }
    }
...
const fn is_right_assoc(op: BinOp) -> bool {
