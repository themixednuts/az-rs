...
pub(super) const fn call_stmt(call: FunctionCall) -> Stmt {
...
pub(super) fn numeric_for(
    ...
    let mut stmt = NumericFor::new(var, start, limit)
        .with_do_token(symbol(" do\n"))
        .with_block(body)
        .with_end_token(symbol("\nend"));
...
pub(super) const fn table_list(value: Expression) -> Field {
    Field::NoKey(value)
}
...
pub(super) const fn name_expr(name: TokenReference) -> Expression {
