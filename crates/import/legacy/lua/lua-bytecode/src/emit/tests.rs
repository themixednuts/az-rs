...
#[test]
fn preserves_statements_after_return_in_a_grammar_block() {
    assert_emit(
        &block(vec![
            Stmt::Return(vec![int(1)]),
            Stmt::Label("later".into()),
        ]),
        concat!("do\n", "\treturn 1\n", "end\n", "::later::\n"),
    );
}
...
#[test]
fn preserves_statements_after_break_in_a_grammar_block() {
    assert_emit(
        &block(vec![Stmt::While {
            cond: Expr::True,
            body: block(vec![Stmt::Break, Stmt::Label("later".into())]),
        }]),
        concat!(
            "while true do\n",
            "\tdo\n",
