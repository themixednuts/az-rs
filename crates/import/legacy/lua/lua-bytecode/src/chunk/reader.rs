...
impl<'a> ByteReader<'a> {
    ...
    pub fn read_number(&mut self) -> Result<f64, LuaError> {
        if self.integral_number {
            ...
            // `integer_fits_f64_exactly` above proves the magnitude is within
            // 2^53, and std has no checked integer-to-float conversion.
...
const fn integer_fits_f64_exactly(value: i64) -> bool {
