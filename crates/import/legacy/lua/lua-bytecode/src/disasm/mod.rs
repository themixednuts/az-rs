...
fn format_instruction(proto: &Proto, table: &OpcodeTable, pc: usize) -> String {
    ...
    if let Some(line_number) = line_number(proto, pc) {
        let stamp = format!("(line {line_number})");
        write!(line, "{stamp:<12}").expect("writing to String cannot fail");
    } else {
        line.push_str(&" ".repeat(12));
    }
    write!(line, "{instruction_text:<42}").expect("writing to String cannot fail");
...
fn format_operands(
    ...
    format_load_operands(proto, inst, pc)
        .or_else(|| format_table_operands(proto, table, inst, pc))
        .or_else(|| format_arithmetic_operands(proto, table, inst, pc))
        .or_else(|| format_branch_operands(proto, table, inst, pc))
        .or_else(|| format_call_operands(proto, inst, pc))
        .or_else(|| format_loop_operands(proto, inst, pc))
        .unwrap_or_else(|| fallback_operands(table, inst))
}

/// Register moves, constant loads, and global/upvalue reads.
fn format_load_operands(proto: &Proto, inst: Instruction, pc: usize) -> Option<(String, String)> {
    let formatted = match inst.op {
        SemanticOp::Move => (
            format!(
                "{}  {}",
...
/// Binary and unary arithmetic, and string concatenation.
fn format_arithmetic_operands(
    proto: &Proto,
    table: &OpcodeTable,
    inst: Instruction,
    pc: usize,
...
/// Unconditional jumps, comparisons, and the test opcodes.
...
fn format_call_operands(proto: &Proto, inst: Instruction, pc: usize) -> Option<(String, String)> {
    let formatted = match inst.op {
        SemanticOp::Call => {
...
/// Numeric and generic `for` loops, list stores, closures, and varargs.
fn format_loop_operands(proto: &Proto, inst: Instruction, pc: usize) -> Option<(String, String)> {
    let formatted = match inst.op {
        SemanticOp::ForLoop => {
            let target = jump_target(pc, inst.sbx);
            (
...
fn k_str(proto: &Proto, index: i32) -> String {
    get_constant(proto, index).map_or_else(
        || format!("K{index}"),
        |constant| format!("K{index}({})", format_constant(constant)),
    )
}
...
fn escape_bytes(bytes: &[u8]) -> String {
    ...
    for &byte in bytes {
        match byte {
            ...
            _ => write!(out, "\\{byte:03}").expect("writing to String cannot fail"),
        }
    }
    out
}

const fn binary_symbol(op: SemanticOp) -> &'static str {
    match op {
        SemanticOp::Add => "+",
        SemanticOp::Sub => "-",
        SemanticOp::Mul => "*",
...
const fn unary_symbol(op: SemanticOp) -> &'static str {
    match op {
        SemanticOp::Unm => "-",
        SemanticOp::Not => "not ",
        SemanticOp::Len => "#",
...
const fn comparison_symbol(op: SemanticOp) -> &'static str {
    match op {
        SemanticOp::Eq => "==",
        SemanticOp::Lt => "<",
        SemanticOp::Le => "<=",
