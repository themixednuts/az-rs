...
pub fn lua51_number_literal(value: f64) -> Option<String> {
    if value.is_nan() {
        return None;
    }
    if !value.is_finite() {
...
fn shorter_exact_literal(value: f64, decimal: String, scientific: String) -> String {
    ...
    match (decimal_exact, scientific_exact) {
        ...
        // Decimal wins whenever it round-trips, and is also the fallback
        // when neither form does.
        (true, _) | (false, false) => decimal,
