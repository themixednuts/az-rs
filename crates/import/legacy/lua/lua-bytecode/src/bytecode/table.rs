...
impl InstructionFormat {
    fn from_name(name: &str) -> Self {
        match name.trim().to_ascii_uppercase().as_str() {
            ...
            "ISJ" => Self::IsJ,
            "IVABC" => Self::IvAbc,
            // "ABC" is also the fallback for an unrecognised format name.
            _ => Self::Abc,
        }
    }
}
...
impl OpcodeTable {
    ...
    #[must_use]
    pub const fn raw_opcode(&self, raw: u32) -> usize {
    ...
    /// Decode the A field.
    #[must_use]
    pub const fn decode_a(&self, raw: u32) -> i32 {
        decode_field(raw, self.op_bits, self.a_bits)
    }

    /// Decode the B field.
    #[must_use]
    pub const fn decode_b(&self, raw: u32) -> i32 {
    ...
    /// Decode the C field.
    #[must_use]
    pub const fn decode_c(&self, raw: u32) -> i32 {
        let shift = if self.has_k_flag {
    ...
    #[must_use]
    pub const fn is_k(&self, field: i32) -> bool {
        !self.has_k_flag && (field & self.rk_bit) != 0
    }

    /// Strip the RK bit to get the constant index.
    #[must_use]
    pub const fn rk_index(&self, field: i32) -> i32 {
        if self.has_k_flag {
            field
        } else {
            field & (self.rk_bit - 1)
...
const fn decode_field(raw: u32, shift: u8, bits: u8) -> i32 {
    // A bit reinterpretation, not a numeric conversion: the field is at most
    // 32 bits wide and callers apply the signed bias themselves.
    ((raw >> shift) & bit_mask(bits)).cast_signed()
}
...
const fn signed_bias(bits: u8) -> i32 {
    // The bias is the low `bits - 1` ones. Opcode field widths are 1..=32, so
    // it always fits in `i32`; anything else is a malformed table row.
    if bits == 0 || bits > 32 {
        return 0;
    }
    i32::MAX >> (32 - bits)
