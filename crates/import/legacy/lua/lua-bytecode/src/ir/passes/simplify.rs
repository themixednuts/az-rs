...
fn fold_binary(op: BinOp, left: f64, right: f64) -> Option<SsaLiteral> {
    let value = match op {
        ...
        // Not constant-folded: bitwise and integer-division ops need target
        // semantics, and `%` by zero has none.
        BinOp::Pow
        | BinOp::IDiv
        | BinOp::BAnd
        | BinOp::BOr
    ...
    match op {
        ...
        UnOp::Len => match value {
            SsaLiteral::Str(value) => match target {
                crate::version::LuaTarget::V51 => {
                    // A string length is far below 2^53, so this converts
                    // exactly; std has no checked integer-to-float conversion.
                    #[allow(clippy::cast_precision_loss)]
                    let len = value.len() as f64;
                    Some(SsaLiteral::number(len))
                }
            },
            _ => None,
        },
...
const fn is_truthy(value: &SsaLiteral) -> bool {
...
const fn changed_if(changed: bool) -> PassChange {
    if changed {
