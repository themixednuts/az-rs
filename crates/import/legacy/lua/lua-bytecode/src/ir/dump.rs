...
fn write_node(out: &mut String, node: &SsaNode, pad: &str) {
    ...
    let handled = write_value_op(out, &node.op)
        || write_arithmetic_op(out, &node.op)
        || write_flow_op(out, &node.op);
    debug_assert!(handled, "every SsaOp variant must have a dump form");
    writeln!(out).expect("writing to String cannot fail");
}

/// Phi nodes, loads, and table/global/upvalue accesses.
///
/// Returns `false` when `op` belongs to another group.
fn write_value_op(out: &mut String, op: &SsaOp) -> bool {
    match op {
...
/// Arithmetic, concatenation, and the jump/branch terminators.
///
/// Returns `false` when `op` belongs to another group.
fn write_arithmetic_op(out: &mut String, op: &SsaOp) -> bool {
    match op {
...
/// Calls, returns, loop headers, list stores, and closures.
///
/// Returns `false` when `op` belongs to another group.
fn write_flow_op(out: &mut String, op: &SsaOp) -> bool {
    match op {
        SsaOp::Call {
            func,
            args,
...
const fn kind_name(op: &SsaOp) -> &'static str {
    match op {
        SsaOp::Nop => "NOP",
        SsaOp::Phi { .. } => "PHI",
        SsaOp::Move { .. } => "MOVE",
...
const fn un_symbol(op: UnOp) -> &'static str {
    match op {
        UnOp::Neg => "-",
        UnOp::Not => "not",
        UnOp::Len => "#",
...
const fn rel_symbol(op: RelOp) -> &'static str {
    match op {
        RelOp::Eq => "==",
        RelOp::Lt => "<",
        RelOp::Le => "<=",
