...
impl<'a> PassContext<'a> {
    const fn new(constants: &'a [Constant]) -> Self {
        Self {
            constants,
            use_counts: None,
            definitions: None,
    ...
    #[must_use]
    pub const fn constants(&self) -> &'a [Constant] {
        self.constants
    }
...
/// Rounds a fixpoint-scheduled pass may repeat before the pipeline gives up.
const FIXPOINT_ROUNDS: NonZeroU8 = match NonZeroU8::new(8) {
    Some(rounds) => rounds,
    None => unreachable!(),
};
...
pub fn cleanup_pipeline() -> PassPipeline {
    ...
    let fixpoint = PassSchedule::fixpoint(FIXPOINT_ROUNDS);
    pipeline
