...
use az_lua_bytecode::{DecompOptions, bytecode::OpcodeTable, chunk::Chunk, ir};
...
fn main() {
    let iterations = std::env::var("AZ_LUA_BENCH_ITERS")
        .ok()
    ...
    let results = [
        measure("full_decompile", iterations, SAMPLES.len(), || {
            for sample in SAMPLES {
                let source = az_lua_bytecode::decompile(black_box(sample.bytes))
        ...
        measure("parse_chunk", iterations, SAMPLES.len(), || {
            for sample in SAMPLES {
                let chunk = az_lua_bytecode::parse_chunk(black_box(sample.bytes))
                    .unwrap_or_else(|err| panic!("{}: {err}", sample.name));
                black_box(chunk);
            }
        }),
        ...
        measure("decompile_ast_core", iterations, ssa.len(), || {
            for item in &ssa {
                let block = az_lua_bytecode::decompile::decompile_proto_with_options(
        ...
        measure("emit_to_source", iterations, blocks.len(), || {
            for item in &blocks {
                let source = az_lua_bytecode::emit::to_source(black_box(&item.block))
    ...
    println!("az-lua decompile hot path benchmark");
...
struct AstSample {
    name: &'static str,
    block: az_lua_bytecode::decompile::ast::Block,
}
...
fn parsed_samples() -> Vec<ParsedSample> {
    SAMPLES
        .iter()
        .map(|sample| {
            let chunk = az_lua_bytecode::parse_chunk(sample.bytes)
...
fn print_result(result: &BenchResult) {
    ...
    println!(
        ...
        per_file(counter(result.allocs), files),
        per_file(counter(result.alloc_bytes), files),
        per_file(counter(result.reallocs), files),
        per_file(counter(result.realloc_bytes), files),
    );
}

/// Widen a benchmark counter for reporting.
///
/// Allocation counts and byte totals never approach 2^53, and std has no
/// checked integer-to-float conversion.
#[allow(clippy::cast_precision_loss)]
const fn counter(value: u64) -> f64 {
    value as f64
}
