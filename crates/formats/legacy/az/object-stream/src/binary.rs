...
#[derive(Debug)]
pub struct BinaryElementHeader<'a> {
    pub(crate) flags: u8,
...
#[derive(Debug)]
pub enum BinaryElement<'a> {
    Header(BinaryElementHeader<'a>),
    EndOfList,
}

pub fn read_stream_header<R: Read>(reader: &mut R) -> Result<u32, ObjectStreamError> {
    let mut buf = [0u8; 1];
...
pub fn read_element_header<'a, R: Read>(
    reader: &mut R,
    stream_version: u32,
    ...
    let (name_crc, field) = if flags & ST_BINARYFLAG_HAS_NAME != 0 {
        reader.read_exact(&mut buf[..4])?;
        let crc = u32::from_be_bytes(buf[..4].try_into().expect("slice width is four"));
        (Some(crc), hashes.and_then(|h| h.field_name(crc)))
    } else {
        (None, None)
    };

    let version = if flags & ST_BINARYFLAG_HAS_VERSION != 0 {
        reader.read_exact(&mut buf[..1])?;
        Some(u32::from(buf[0]))
    } else {
        None
    ...
    let specialization = if stream_version == 2 {
        reader.read_exact(&mut buf)?;
        Some(Uuid::from_slice(&buf)?)
    } else {
        None
    };
...
pub fn ensure_reader_exhausted<R: Read>(reader: &mut R) -> Result<(), ObjectStreamError> {
    let mut byte = [0u8; 1];
...
const fn validate_element_flags(flags: u8) -> Result<(), ObjectStreamError> {
