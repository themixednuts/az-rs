...
/// # Errors
///
/// Returns [`ParseError::UnexpectedEof`] when the buffer ends mid-field (the
/// `what` payload names the field), [`ParseError::UnsupportedVersion`] when the
/// leading version word is not `0`, [`ParseError::ByteLengthOverflow`] when a
/// declared count does not fit in `usize` or `count * 12` overflows,
/// [`ParseError::InvalidUtf8`] when a metadata key or value is not valid UTF-8,
/// and [`ParseError::TrailingBytes`] when bytes remain after the final reserved
/// word.
pub fn parse_vertex_shape_asset(bytes: &[u8]) -> Result<VertexShapeAsset<'_>, ParseError> {
    ...
    let reserved = VertexShapeReserved::new(reserved_first, reserved_second, reserved_third);
...
fn read_metadata<'a>(
    ...
    for _ in 0..count {
        let key = read_string(bytes, cursor, "metadata key")?;
        let value = read_string(bytes, cursor, "metadata value")?;
        metadata.push(VertexShapeMetadata { key, value });
    }
    Ok(metadata)
}
...
mod tests {
    ...
    #[test]
    fn parses_vertex_shape_without_allocating_tables() {
        ...
        assert_eq!(
            asset.vertices().iter().collect::<Vec<_>>(),
            vec![Vec3::new(1.0, 2.0, 0.0), Vec3::new(4.0, 5.0, 0.0)]
        );
        assert_eq!(
            asset.metadata(),
            &[VertexShapeMetadata {
        ...
        // Bit-exact: the fixture encodes this f32 verbatim, so the round-trip
        // must reproduce every bit, not merely land within a tolerance.
        assert_eq!(asset.height().to_bits(), 64.0f32.to_bits());
    }
    ...
    fn rejects_trailing_bytes() {
        ...
        assert_eq!(
            parse_vertex_shape_asset(&bytes).unwrap_err(),
            ParseError::TrailingBytes { remaining: 1 }
        );
    }
    ...
    fn write_string(bytes: &mut Vec<u8>, value: &str) {
        let len = u32::try_from(value.len()).expect("test fixture string fits in u32");
