...
impl MannequinXmlKind {
    ...
    pub fn from_source_path(source_path: &str) -> Option<Self> {
        ...
        if std::path::Path::new(&normalized)
            .extension()
            .is_some_and(|ext| ext.eq_ignore_ascii_case("adb"))
        {
...
impl MannequinAnimationDatabaseSource {
    /// # Errors
    ///
    /// Returns [`MannequinAnimationDatabaseSourceError`] if `bytes` is not a
    /// well-formed Mannequin `.adb` document.
    pub fn from_legacy(
        source_path: &str,
        bytes: &[u8],
    ...
    /// # Errors
    ///
    /// Returns a [`ron::error::SpannedError`] if `bytes` is not a valid
    /// serialized [`MannequinAnimationDatabaseSource`].
...
impl MannequinTagDefinitionSource {
    /// # Errors
    ///
    /// Returns [`MannequinXmlParseError`] if `bytes` is not a well-formed
    /// Mannequin tag-definition XML document.
    ...
    ///
    /// Returns a [`ron::error::SpannedError`] if `bytes` is not a valid
    /// serialized [`MannequinTagDefinitionSource`].
    pub fn from_ron_bytes(bytes: &[u8]) -> Result<Self, ron::error::SpannedError> {
        ron::de::from_bytes(bytes)
    }
}
...
impl MannequinControllerDefinitionSource {
    ...
    ///
    /// Returns [`MannequinXmlParseError`] if `bytes` is not a well-formed
    /// Mannequin controller-definition XML document.
    pub fn from_legacy(source_path: &str, bytes: &[u8]) -> Result<Self, MannequinXmlParseError> {
    ...
    ///
    /// Returns a [`ron::error::SpannedError`] if `bytes` is not a valid
    /// serialized [`MannequinControllerDefinitionSource`].
    pub fn from_ron_bytes(bytes: &[u8]) -> Result<Self, ron::error::SpannedError> {
        ron::de::from_bytes(bytes)
    }
}
...
impl MannequinSourceTransform {
    /// # Errors
    ///
    /// Returns [`MannequinSourceTransformError`] if `input` is not a
    /// supported or well-formed Mannequin source document.
    pub fn transform(
        &self,
    ...
    where
        F: FnMut(&str) -> Option<String>,
    {
        let MannequinSourceInput {
            source_path,
            bytes: document,
        } = input;
        let kind = MannequinXmlKind::from_source_path(&source_path).ok_or_else(|| {
        ...
        let bytes = match kind {
            ...
            MannequinXmlKind::Actions | MannequinXmlKind::Tags => {
                MannequinTagDefinitionSource::from_legacy(&source_path, document)?.to_ron_bytes()?
            }
            MannequinXmlKind::ControllerDefinition => {
                MannequinControllerDefinitionSource::from_legacy(&source_path, document)?
                    .to_ron_bytes()?
            }
        };

        Ok(MannequinSourceArtifact {
            path: kind.source_path(&source_path),
...
pub use az_filesystem::normalize_source_path;
...
impl MannequinAnimationDatabaseBuilder {
        ...
        match item {
            AnimationDatabaseItem::Database(database) => {
                self.database = Some(MannequinAnimationDatabase {
                    definition: database.definition.map(std::borrow::Cow::into_owned),
                    fragment_definition: database
                        .fragment_definition
                        .map(std::borrow::Cow::into_owned),
                    tag_definition: database.tag_definition.map(std::borrow::Cow::into_owned),
            ...
            AnimationDatabaseItem::FragmentBlend(blend) => {
                ...
                database.fragment_blends.push(MannequinFragmentBlend {
                    from: blend.from.map(std::borrow::Cow::into_owned),
                    to: blend.to.map(std::borrow::Cow::into_owned),
                    variants: Vec::new(),
                });
    ...
    fn push_sub_database(
        ...
        database.sub_databases.push(MannequinSubDatabase {
            ...
            tags: sub_database.tags.map(std::borrow::Cow::into_owned),
    ...
    fn push_procedural(
        ...
        procedurals.push(MannequinProcedural {
            ...
            context_type: procedural.context_type.map(std::borrow::Cow::into_owned),
            parameters: Vec::new(),
        });
    ...
    fn push_procedural_parameter(
        ...
        let new_index = {
            ...
            children.push(MannequinProceduralParameter {
                ...
                value: parameter.value.map(std::borrow::Cow::into_owned),
                children: Vec::new(),
            });
            children.len() - 1
        };
    ...
    fn push_fragment_blend_variant(
        ...
        database.fragment_blends[blend]
            ...
            .push(MannequinFragmentBlendVariant {
                from: variant.from.map(std::borrow::Cow::into_owned),
                to: variant.to.map(std::borrow::Cow::into_owned),
                from_fragment: variant.from_fragment.map(std::borrow::Cow::into_owned),
                to_fragment: variant.to_fragment.map(std::borrow::Cow::into_owned),
                fragments: Vec::new(),
            });
...
impl From<FragmentRef<'_>> for MannequinFragment {
    fn from(fragment: FragmentRef<'_>) -> Self {
        Self {
            ...
            tags: fragment.tags.map(std::borrow::Cow::into_owned),
            fragment_tags: fragment.fragment_tags.map(std::borrow::Cow::into_owned),
            select_time: fragment.select_time,
            start_time: fragment.start_time,
            enter_time: fragment.enter_time,
            flags: fragment.flags.map(std::borrow::Cow::into_owned),
...
impl From<AnimationEntryRef<'_>> for MannequinAnimationEntry {
    fn from(animation: AnimationEntryRef<'_>) -> Self {
        Self {
            ...
            flags: animation.flags.map(std::borrow::Cow::into_owned),
...
fn parse_controller_definition_source(
    ...
    loop {
        match reader.read_event()? {
            ...
            Event::Empty(event) => {
                let element = state.visit_start(&reader, &event)?;
                ControllerDefinitionParseState::visit_end(element);
            }
            Event::End(_) => {
                let element = state
                    .stack
...
impl ControllerDefinitionParseState {
    ...
    const fn visit_end(_element: ControllerDefinitionElement) {}
...
mod tests {
    ...
    fn transforms_animation_database_to_authoring_source() {
        let output = MannequinSourceTransform
            .transform(MannequinSourceInput::new(
                "Animations/Mannequin/ADB/Player/combat.adb",
                sample_animation_database(),
            ))
            .unwrap();
