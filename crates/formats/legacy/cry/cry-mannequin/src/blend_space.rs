...
//! `dev/Gems/CryLegacy/Code/Source/CryAnimation/GlobalAnimationHeaderLMG.cpp`
//! and the editor structures in
//! `dev/Code/Sandbox/Plugins/EditorAnimation/CharacterTool/BlendSpace.h`.
...
impl BlendSpaceXmlKind {
    ...
    pub fn from_source_path(source_path: &str) -> Option<Self> {
        ...
        if normalized.ends_with(".bspace") {
            ...
            .extension()
            .is_some_and(|ext| ext.eq_ignore_ascii_case("comb"))
        {
...
impl BlendSpaceSource {
    /// # Errors
    ///
    /// Returns [`BlendSpaceSourceError`] if `bytes` is not a well-formed
    /// `ParaGroup` blend-space document.
...
/// A legacy blend-space document classified by its serialized root rather
/// than its filename extension.
///
/// Legacy assets do not always keep `.comb` and `.bspace`
/// aligned with `CombinedBlendSpace` and `ParaGroup` roots.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
impl BlendSpaceDocumentSource {
    ...
    ///
    /// Returns [`BlendSpaceSourceError`] if `bytes` is not a well-formed
    /// `ParaGroup` or `CombinedBlendSpace` document.
    pub fn from_legacy(source_path: &str, bytes: &[u8]) -> Result<Self, BlendSpaceSourceError> {
...
impl CombinedBlendSpaceSource {
    ...
    ///
    /// Returns a [`ron::error::SpannedError`] if `bytes` is not a valid
    /// serialized [`CombinedBlendSpaceSource`].
    pub fn from_ron_bytes(bytes: &[u8]) -> Result<Self, ron::error::SpannedError> {
        ron::de::from_bytes(bytes)
    }
...
impl<'a> BlendSpaceSourceInput<'a> {
    #[must_use]
...
impl BlendSpaceSourceTransform {
    /// # Errors
    ///
    /// Returns [`BlendSpaceSourceTransformError`] if `input` is not a
    /// supported or well-formed blend-space source document.
    pub fn transform(
        &self,
    ...
    where
        F: FnMut(&str) -> Option<String>,
    {
        let BlendSpaceSourceInput {
            source_path,
            bytes: document,
        } = input;
        let source_path = source_path.into_string();
        if BlendSpaceXmlKind::from_source_path(&source_path).is_none() {
            return Err(BlendSpaceSourceTransformError::UnsupportedPath { path: source_path });
        }
        let artifact_source_path = source_path.clone();

        let (kind, bytes) = match parse_blend_space_document(document)? {
        ...
        Ok(BlendSpaceSourceArtifact {
            path: kind.source_path(&artifact_source_path),
...
fn parse_blend_space_document(bytes: &[u8]) -> Result<BlendSpaceDocument, BlendSpaceSourceError> {
    ...
    loop {
        match reader.read_event()? {
            ...
            Event::Empty(event) => {
                let element = state.visit_start(&reader, &event)?;
                BlendSpaceParseState::visit_end(element);
            }
            Event::End(_) => {
...
impl BlendSpaceParseState {
    ...
    const fn visit_end(_element: BlendSpaceElement) {}
...
fn attr_i32_index_required(
    ...
    attr_i32_index(reader, event, prefix, index)?.ok_or_else(|| {
...
fn attr_f32_index_required(
    ...
    attr_f32_index(reader, event, prefix, index)?.ok_or_else(|| {
        BlendSpaceSourceError::MissingIndexedAttribute {
            prefix: String::from_utf8_lossy(prefix).into_owned(),
            index,
        }
...
mod tests {
    ...
    fn transforms_parametric_blend_space_to_authoring_ron() {
        let artifact = BlendSpaceSourceTransform
            .transform_with_motion_resolver(
                BlendSpaceSourceInput::new(
                    "Animations/Gameplay/Character/Npc/Mounts/Grunt/Blendspace/mount_grunt_movement_walk_blend.bspace",
                    sample_bspace(),
                ),
                |name| match name {
    ...
    fn transforms_combined_blend_space_to_authoring_ron() {
        let artifact = BlendSpaceSourceTransform
            .transform(BlendSpaceSourceInput::new(
 ...
 <BlendSpaces>
  ...
  <BlendSpace AName="animations/test_right.bspace"/>
    ...
    fn transform_accepts_parametric_blend_space_with_comb_extension() {
        let artifact = BlendSpaceSourceTransform
            .transform(BlendSpaceSourceInput::new(
                "animations/scorpion_turn_comb_blend.comb",
                br#"<ParaGroup>
