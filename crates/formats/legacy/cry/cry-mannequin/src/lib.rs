...
//!
//! Follows Lumberyard's `dev/Gems/CryLegacy/Code/Source/CryAction/Mannequin/AnimationDatabaseManager.cpp`.
...
pub use fragment_audio::{
    AUDIO_PROCEDURAL_TYPE, DO_NOTHING_TRIGGER, MannequinAudioClip, MannequinFragmentAudio,
};
...
///
/// Defined by Lumberyard's `dev/Code/CryEngine/CryCommon/ICryMannequin.h`.
...
/// # Errors
///
/// Returns [`AnimationDatabaseParseError`] if `bytes` is not a well-formed
/// Mannequin `.adb` document.
...
impl AnimationDatabaseTotals {
    pub const fn add_stats(&mut self, stats: AnimationDatabaseStats) {
        self.files += 1;
...
/// # Errors
///
/// Returns [`AnimationDatabaseInspectionError::Read`] if `path` cannot be
/// read, or [`AnimationDatabaseInspectionError::Parse`] if its contents are
/// not a well-formed Mannequin `.adb` document.
...
/// # Errors
///
/// Returns [`AnimationDatabaseInspectionError`] on the first path that
/// cannot be read or parsed as a Mannequin `.adb` document.
...
#[must_use]
pub const fn is_animation_database_extension(extension: &str) -> bool {
...
///
/// Returns [`AnimationDatabaseParseError`] if `bytes` is not a well-formed
/// Mannequin `.adb` document, or if `visitor` returns an error.
...
impl AnimationDatabaseState {
        ...
        F: FnMut(AnimationDatabaseItem<'_>) -> Result<(), AnimationDatabaseParseError>,
    {
        ...
        let kind = if self.stack.is_empty() {
            ...
            match parent {
                ...
                Some(ElementKind::ProceduralParams | ElementKind::ProceduralParameter) => {
                ...
                Some(
                    ElementKind::LayerBlend
                    | ElementKind::Animation
                    | ElementKind::SubDatabaseFragment
                    | ElementKind::Unknown,
                )
    ...
    const fn visit_end(&mut self, element: ElementKind) {
