...
impl PakExtractReport {
    #[inline]
    #[must_use]
    pub const fn selected_entries(&self) -> usize {
        self.selected_entries
    }

    #[inline]
    #[must_use]
    pub const fn written_files(&self) -> usize {
        self.written_files
    }

    #[inline]
    #[must_use]
    pub const fn skipped_existing_files(&self) -> usize {
    ...
    /// # Errors
    ///
    /// Returns [`PakExtractFailures`], carrying the number of collected
    /// [`PakExtractEntryFailure`] records, whenever
    /// [`PakExtractReport::has_failures`] is true.
    pub const fn ensure_success(&self) -> Result<(), PakExtractFailures> {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(
            ...
            if self.skipped_existing_files > 0 {
                format!(
                    ", skipped {} existing (pass --force to replace)",
                    self.skipped_existing_files
                )
            } else {
                String::new()
...
impl PakFile {
    ...
    /// # Errors
    ///
    /// Returns [`PakExtractError::CreateOutputRoot`] or
    /// [`PakExtractError::CanonicalizeOutputRoot`] when `output_root` cannot
    /// be created or resolved, and [`PakExtractError::OpenPak`] when the pak
    /// cannot be opened for the entry-name scan or the sequential read pass.
    /// Per-entry failures do not abort the run — they are collected into the
    /// returned [`PakExtractReport`].
    pub fn extract_to_dir(
        pak_path: impl AsRef<Path>,
        output_root: impl AsRef<Path>,
        options: PakExtractOptions<'_>,
    ) -> Result<PakExtractReport, PakExtractError> {
        extract_to_dir(pak_path.as_ref(), output_root.as_ref(), options)
    }
}
...
fn extract_parallel(
    ...
    for name in names {
        ...
        pending.push((name.as_str(), dest));
    }
    if pending.is_empty() {
        return;
    }

    let names_refs: Vec<&str> = pending.iter().map(|(name, _)| *name).collect();
    let results = PakFile::extract_parallel(pak_path, &names_refs);
    for ((name, dest), result) in pending.into_iter().zip(results) {
        match result
            .map_err(|error| error.to_string())
...
mod tests {
    ...
    fn report_formats_success_deterministically() {
        ...
        assert_eq!(
            report.to_string(),
            "3 entries to extract -> out\nwrote 2 files, skipped 1 existing (pass --force to replace)\n"
        );
    }
    ...
    fn wildcard_selector_matches_paths_case_insensitively() {
        assert!(entry_selected(
            "SharedAssets/Example/World/Regions/r_+00_+00/region.slicedata",
            None,
            &["sharedassets/example/world/regions/*/region.slicedata"],
        ));
        assert!(!entry_selected(
            "sharedassets/example/world/regions/r_+00_+00/region.heightmap",
            None,
            &["sharedassets/example/world/regions/*/region.slicedata"],
        ));
    }
