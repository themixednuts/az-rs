//! `AzCore` UUID derivation and RTTI `TypeId` folding.
//!
//! Lumberyard/O3DE `AZ::Uuid::CreateName` / `CreateData` are not
//! RFC 4122 v5 UUIDs because they do not hash an RFC namespace. They
//! SHA-1 the input bytes directly, take the first 16 digest bytes, then
//! apply `AzCore`'s version and variant masks. `AZ::Uuid::operator+` uses
//! the same primitive over `lhs || rhs`; RTTI template `TypeIds` are built
//! from that operator.

use uuid::Uuid;

/// `AzCore` `Uuid::CreateData(data, size)`.
///
/// This hashes `bytes` directly, without an RFC namespace. Empty input
/// returns the nil UUID, matching `AzCore` `CreateNull`.
#[inline]
#[must_use]
const fn create_data(bytes: &[u8]) -> Uuid {
    if bytes.is_empty() {
    ...
    // Source-spelled as AzCore VAR_RFC_4122 and VER_NAME_SHA1 masks.
...
/// `AzCore` `Uuid::CreateName(name)`.
#[inline]
#[must_use]
const fn create_name(name: &[u8]) -> Uuid {
    create_data(name)
}

/// `AzCore` `Uuid::operator+`.
#[inline]
#[must_use]
const fn combine(lhs: Uuid, rhs: Uuid) -> Uuid {
...
/// `AZ::Uuid` static/member surface that Lumberyard/O3DE expose as inherent
/// `AZ::Uuid` functions.
///
/// Rust cannot add inherent associated functions to the foreign [`Uuid`]
/// type, so the `AZ::Uuid::CreateName` / `AZ::Uuid::CreateData` /
/// `AZ::Uuid::operator+` API is provided through this extension trait. Bring it
/// into scope (`use az_core::uuid::AzUuidExt;`) and the call sites read exactly
/// like the C++ originals: `Uuid::create_name(bytes)`,
/// `Uuid::create_data(bytes)`, and `lhs.combine(rhs)`.
pub trait AzUuidExt: Sized {
    /// Mirrors `AZ::Uuid::CreateName(name)`.
    ///
    /// SHA-1 hashes `name` directly — there is no RFC 4122 namespace — then
    /// applies `AzCore`'s historical version/variant masks. Empty input yields
    /// the nil UUID, matching `AZ::Uuid::CreateNull`.
    #[must_use]
    fn create_name(name: &[u8]) -> Self;

    /// Mirrors `AZ::Uuid::CreateData(data, size)`.
    ///
    /// Shares the exact hashing primitive with [`AzUuidExt::create_name`]; the
    /// two `AzCore` entry points differ only in name.
    #[must_use]
    fn create_data(data: &[u8]) -> Self;

    /// Mirrors `AZ::Uuid::operator+`.
    ///
    /// Hashes `lhs || rhs` back into a single UUID; this is the fold used to
    /// build RTTI template `TypeIds`.
    #[must_use]
...
// SHA-1 working state deliberately uses the canonical single-letter register
// names (a..e, plus f/k/w) from FIPS 180-4; renaming them would obscure the
// standard algorithm.
...
// The `as u8` narrowing is the intended byte extraction of a big-endian u32;
// truncation is exactly the desired behavior, not a hazard.
...
// Same FIPS 180-4 register naming as `sha1`; see the note there.
#[allow(clippy::many_single_char_names)]
...
// `current % 10` is provably a single decimal digit (0..=9); the `as u8`
// narrowing to form its ASCII byte can never lose information.
#[allow(clippy::cast_possible_truncation)]
...
pub mod type_ids {
    ...
    pub const AZSTD_STRING_LEGACY_XML: Uuid =
    ...
    pub const AZSTD_VECTOR_LEGACY_XML: Uuid =
...
///
/// Uses the `AZStd::unordered_map` operand order with the flat-map base id.
#[inline]
#[must_use]
...
///
/// The Lumberyard/O3DE `TypeInfo` specialization only folds the pointee type.
#[inline]
#[must_use]
...
///
/// Folds `T` with `CreateName("{Min}, {Max}")`, then appends the ranged-int
/// base id.
#[inline]
#[must_use]
