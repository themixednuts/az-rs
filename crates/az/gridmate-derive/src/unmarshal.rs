    ...
    match fields.style {
        darling::ast::Style::Struct => {
            let field_unmarshals = fields.iter().filter_map(|f| {
                ...
                // Wrap each field in `rb.field(name, |rb| ...)` so that when a
                // DebugRecorder is attached (via ReadBuffer::with_recorder),
                // the recorder captures the byte range and Debug-formatted
                // value of this field. In production rb.field is a direct
                // passthrough — no overhead.
                Some(match (&f.r#as, &f.unmarshal_with) {
                    (Some(as_type), _) => quote! {
                        let #field_name = rb.field(#field_name_str, |rb| {
                            ::core::result::Result::Ok::<_, ::gridmate::serialize::error::MarshalerError>(
                                <#field_type as ::core::convert::From<#as_type>>::from(
                                    <#as_type as ::gridmate::serialize::marshaler::Marshaler>::unmarshal(rb)?,
                                ),
                            )
                        })?;
                    },
                    (None, Some(unmarshal_with)) => quote! {
                        let #field_name = rb.field(#field_name_str, |rb| #unmarshal_with(rb))?;
                    },
                    (None, None) => quote! {
                        let #field_name = rb.field(#field_name_str, |rb| {
                            <#field_type as ::gridmate::serialize::marshaler::Marshaler>::unmarshal(rb)
                        })?;
                    },
                })
            });
        ...
        darling::ast::Style::Tuple => {
            let field_unmarshals = fields.iter().enumerate().filter_map(|(i, f)| {
                ...
                Some(match (&f.r#as, &f.unmarshal_with) {
                    ...
                    (None, Some(unmarshal_with)) => quote! { let #field_name = #unmarshal_with(rb)?; },
                    (None, None) => quote! { let #field_name = <#field_type>::unmarshal(rb)?; },
                })
            });
...
fn generate_enum_unmarshal(
    ...
    let match_arms = variants.iter().zip(discriminants).map(|(variant, discriminant)| {
        ...
        // Tagged-union tags are a single byte on the wire, and the marshal side
        // emits `#discriminant as u8`; take the same low byte here so a tag that
        // overflows `u8` round-trips identically instead of diverging.
        let discriminant = discriminant.to_le_bytes()[0];
    ...
    quote! {
        ...
        match discriminant {
            #(#match_arms)*
            _ => Err(::gridmate::serialize::error::MarshalerError::InvalidDiscriminant { value: discriminant }),
        }
    }
}
    ...
    let unknown_arm = unknown_arm.unwrap_or_else(|| {
        quote! {
            _ => {
                ...
                Err(::gridmate::serialize::error::MarshalerError::InvalidDiscriminant { value })
            },
        }
    });
...
fn enum_discriminants(variants: &[MarshalerVariant]) -> Vec<i128> {
    let mut next = 0_i128;
    variants
        .iter()
        .map(|variant| {
