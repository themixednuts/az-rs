    ...
    match fields.style {
        darling::ast::Style::Struct => {
            let field_marshals = fields.iter().filter_map(|f| {
                ...
                Some(match (&f.r#as, &f.with) {
                    (Some(as_type), _) => {
                        quote! { <#as_type as ::core::convert::From<_>>::from(self.#field_name.clone()).marshal(wb); }
                    }
                    (None, Some(with)) => quote! { #with(&self.#field_name, wb); },
                    (None, None) => quote! { self.#field_name.marshal(wb); },
                })
            });
        ...
        darling::ast::Style::Tuple => {
            let field_marshals = fields.iter().enumerate().filter_map(|(i, f)| {
                ...
                Some(match (&f.r#as, &f.with) {
                    ...
                    (None, Some(with)) => quote! { #with(&self.#index, wb); },
                    (None, None) => quote! { self.#index.marshal(wb); },
                })
            });
            ...
            darling::ast::Style::Tuple => {
                ...
                let field_marshals = variant.fields.iter().zip(&field_names).filter_map(|(f, name)| {
                    ...
                    Some(match (&f.r#as, &f.with) {
                        ...
                        (None, Some(with)) => quote! { #with(#name, wb); },
                        (None, None) => quote! { #name.marshal(wb); },
                    })
                });
