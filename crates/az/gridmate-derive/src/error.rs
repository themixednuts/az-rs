...
//! `compile_error!`-shaped diagnostics with source spans. Helper code
...
pub fn darling_to_syn(err: darling::Error) -> syn::Error {
    ...
    let span = err.span();
    // `syn::Error::new` takes the message as `impl Display`, so the darling
    // error is consumed directly rather than stringified first.
