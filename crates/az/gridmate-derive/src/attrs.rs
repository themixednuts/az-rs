// darling's `FromDeriveInput` / `FromField` expansions land here as sibling
// impls carrying this file's spans, so their generated parse loops and
// `#[darling(default)]` fallbacks can only be exempted at module scope.
...
impl FromVariant for MarshalerVariant {
    fn from_variant(variant: &syn::Variant) -> darling::Result<Self> {
        ...
        let mut discriminant: Option<i128> = match &variant.discriminant {
            Some((_, expr)) => Some(parse_integer_expr(expr)?),
            None => None,
        };
        ...
        for attr in &variant.attrs {
            ...
            attr.parse_nested_meta(|meta| {
                if meta.path.is_ident("discriminant") {
                    ...
                    discriminant = Some(parse_integer_expr(&expr)?);
...
impl MarshalerVariant {
    ...
    pub fn discriminant_value(&self, default_index: i128) -> i128 {
        self.discriminant.unwrap_or(default_index)
    }
}

fn parse_integer_expr(expr: &Expr) -> darling::Result<i128> {
    match expr {
        Expr::Group(expr) => parse_integer_expr(&expr.expr),
        Expr::Paren(expr) => parse_integer_expr(&expr.expr),
        Expr::Lit(expr) => match &expr.lit {
            syn::Lit::Int(lit) => lit.base10_parse::<i128>().map_err(darling::Error::from),
            _ => Err(integer_literal_error()),
        },
...
fn integer_literal_error() -> darling::Error {
    darling::Error::custom("Marshaler enum discriminants must be integer literals")
}
