...
impl BlockingPipelineConfig {
    ...
    pub fn automatic() -> Self {
        Self {
            ...
            thread_name_prefix: "az-jobs-pipeline".to_string(),
        }
    }
    ...
    pub fn with_thread_name_prefix(mut self, prefix: impl Into<String>) -> Self {
        ...
        if self.thread_name_prefix.is_empty() {
            self.thread_name_prefix = "az-jobs-pipeline".to_string();
        }
        self
    }
...
// `cancel` is a cheap-clone token (Arc/atomic-backed) taken by value by design:
// the pipeline fans out clones to the producer and every worker thread, so
// ownership transfer at the call boundary is the natural API, not an
// unnecessary copy.
    ...
    C: Cancellation,
{
    ...
    Ok(BlockingPipelineSummary {
        produced: produced_count.load(Ordering::Relaxed),
        consumed,
    ...
    thread::Builder::new()
        ...
        .spawn(move || {
            loop {
                ...
                let Some(item) = recv_input(&cancel, &input_rx) else {
                    break;
                };
...
where
    C: Cancellation,
{
    let cancellation = cancel.cancellation_signal();
    select_biased! {
        recv(cancellation.receiver()) -> _ => false,
        send(tx, item) -> result => result.is_ok(),
    }
}
    ...
    C: Cancellation,
{
    ...
    select_biased! {
        recv(cancellation.receiver()) -> _ => false,
        send(tx, output) -> result => result.is_ok(),
    }
}
...
mod tests {
    ...
    fn blocking_pipeline_streams_outputs_to_consumer() {
        ...
        let summary = try_for_each_bounded_blocking(
            &BlockingPipelineConfig::new(2)
                ...
                .with_thread_name_prefix("az-jobs-test"),
            cancel,
    ...
    fn blocking_pipeline_uses_each_worker() {
        ...
        let all_named_correctly = seen
            .lock()
            .unwrap()
            .iter()
            .all(|name| name.starts_with("az-jobs-pipeline-worker-"));
        assert!(all_named_correctly);
    }
