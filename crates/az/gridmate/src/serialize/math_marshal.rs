...
use az_core::math::Bounds;
use bevy_color::LinearRgba;
use bevy_math::bounding::{Aabb2d, Aabb3d};
use bevy_math::{Affine3A, Mat3, Mat3A, Quat, Vec2, Vec3, Vec3A, Vec4};
...
impl Marshaler for LinearRgba {
    const MARSHAL_SIZE: usize = 16;

    fn marshal(&self, wb: &mut WriteBuffer) {
        self.red.marshal(wb);
        self.green.marshal(wb);
        self.blue.marshal(wb);
        self.alpha.marshal(wb);
    }

    fn unmarshal(rb: &mut ReadBuffer) -> Result<Self, MarshalerError> {
        Ok(Self {
            red: f32::unmarshal(rb)?,
            green: f32::unmarshal(rb)?,
            blue: f32::unmarshal(rb)?,
            alpha: f32::unmarshal(rb)?,
        })
    }
}
...
/// `AZ::Quaternion` wire layout: four raw `f32` components in xyzw order —
/// 16 bytes. Mirrors Lumberyard's `Marshaler<AZ::Quaternion>` (uncompressed
...
/// `AZ::Matrix3x3` wire layout: three column basis vectors as raw `Vec3` —
/// 36 bytes. Mirrors Lumberyard's `Marshaler<AZ::Matrix3x3>` which calls
...
impl Marshaler for Mat3 {
    ...
    fn unmarshal(rb: &mut ReadBuffer) -> Result<Self, MarshalerError> {
        ...
        Ok(Self::from_cols(x_axis, y_axis, z_axis))
    }
}

/// `AZ::Transform` wire layout: three column basis vectors plus translation
/// as raw `Vec3` — 48 bytes total. Mirrors Lumberyard's
/// `Marshaler<AZ::Transform>` which emits `GetBasisX/Y/Z` followed by
/// `GetTranslation()`. glam's `Affine3A` (= `Mat3A` + `Vec3A`) is the
/// equivalent SIMD-aligned affine; we round-trip through `Vec3` for the
/// per-axis reads to keep the wire shape identical to the C++ source.
...
/// `AZ::Aabb` wire layout: `(min.x, min.y, min.z, max.x, max.y, max.z)`,
/// six raw `f32` values. The marshaler converts Bevy's SIMD-aligned `Vec3A`
/// fields to `Vec3` so their padding never reaches the wire.
...
mod tests {
    ...
    fn bounds_marshal_is_two_vec2_16_bytes() {
        let bounds = Bounds::new(Vec2::new(1.0, 2.0), Vec2::new(3.0, 4.0));
        let mut wb = WriteBuffer::new(Endian::BigEndian);
        bounds.marshal(&mut wb);
        let bytes = wb.into_vec();
        assert_eq!(bytes.len(), 16);
        let mut rb = ReadBuffer::new(Endian::BigEndian, &bytes);
        let decoded = Bounds::unmarshal(&mut rb).unwrap();
        assert_eq!(decoded, bounds);
    }

    #[test]
    fn aabb2d_marshal_is_two_vec2_16_bytes() {
        let aabb = Aabb2d {
        ...
        assert_eq!(decoded, aabb);
    }

    #[test]
    fn linear_rgba_marshal_is_four_f32_16_bytes() {
        let color = LinearRgba::new(0.1, 0.2, 0.3, 0.4);
        let mut wb = WriteBuffer::new(Endian::BigEndian);
        color.marshal(&mut wb);
        let bytes = wb.into_vec();
        assert_eq!(bytes.len(), 16);
        let mut rb = ReadBuffer::new(Endian::BigEndian, &bytes);
        let decoded = LinearRgba::unmarshal(&mut rb).unwrap();
