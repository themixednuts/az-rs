//! `MaskChain` — a u8 chain with a continuation bit.
//!
...
//! chain"; clear means "terminator." Used by native base-state sub-field
...
impl MaskChain {
    ...
    /// # Errors
    ///
    /// Returns [`MarshalerError::BufferUnderrun`] if the buffer is exhausted
    /// before a byte with the continuation bit clear terminates the chain — a
    /// truncated or malformed chain never reaches its terminator.
