...
//! wire shape used by replicated-field collections.
//!
...
//! revision tags) are always emitted; the value body is only present when
//! the bit is set.
//!
...
/// # Errors
///
/// Returns [`MarshalerError::BufferUnderrun`] if the buffer runs out before a
/// chunk's live-mask byte, and any error `f` returns while decoding an entry
/// body.
