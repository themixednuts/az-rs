...
pub enum MarshalerError {
    ...
    ArrayStringOverflow { len: usize, capacity: usize },
    ...
    /// A polymorphic value field referenced a compact type id that is not in
    /// the Rust `TypeRegistry` class registrations. Either the type has not
    /// been ported yet, or the wire is corrupted.
    ...
    /// variant of the receiving `NetworkBase`. The default
    /// `NetworkBase::unmarshal_body` raises this; the
    /// `#[derive(NetworkBase)]` macro overrides the default when an
    /// `Unknown` variant is present.
    #[error("polymorphic value carried a UUID with no matching NetworkBase variant")]
    UnknownClassUuid,

    /// The read reached a polymorphic field, which needs the host's composed
    /// class registry to turn a compact `type_index` into a concrete type, but
    /// the buffer was never bound to one
    /// ([`ReadBuffer::with_types`](crate::serialize::buffer::ReadBuffer::with_types)).
    ///
    /// This is a wiring bug at the seam that built the buffer, not bad wire
    /// data: an unbound buffer fails loudly here instead of silently reporting
    /// every reflected type as unknown.
    #[error("read buffer is not bound to a composed class registry")]
    UnboundTypes,
}
...
impl MarshalerError {
    #[inline]
    #[must_use]
    pub const fn buffer_underrun(available: usize, needed: usize) -> Self {
