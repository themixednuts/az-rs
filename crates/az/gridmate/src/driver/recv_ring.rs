...
impl RecvRing {
    ...
    /// # Panics
    ///
    /// Panics if `chunk_size < max_datagram` — a chunk that cannot hold one
    /// full datagram would roll over on every recv and never make progress.
    // The rings are constructed by the DTLS connection paths, which are gated
    // on `client` / `server`; a `transport`-only build reaches this only from
