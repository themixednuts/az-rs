...
//! drain system) belongs in the embedding server.
...
/// Identity of one DTLS session at a peer address.
///
/// Some clients reuse their fixed UDP endpoint across reconnects. The
/// address alone therefore cannot distinguish a late event or cleanup from an
/// old session from the current session at that address.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) struct PeerGeneration(u64);
...
impl MultiPeerListener {
    ...
    /// # Errors
    ///
    /// Returns [`DriverError::Address`] if `addr` is not a parseable
    /// `host:port`, [`DriverError::Io`] if the UDP socket cannot be
    /// bound to it or cannot be put into non-blocking mode, and
    /// [`DriverError::Ssl`] if `cert_pem` / `key_pem` are not valid PEM
    /// or the server `SSL_CTX` rejects them. The Windows
    /// `SIO_UDP_CONNRESET` ioctl is best-effort and never fails the
    /// bind.
    // `async` here is a deliberate public API contract, not an
    // oversight: this is the server entry point, and callers across the
    // workspace are written against an async `bind`. Removing it changes
    // the public signature — it does not even compile on its own, since
    // dropping the `.await` in `ServerListenerHandle::bind_configured`
    // leaves both it and `ServerListenerHandle::bind` awaitless in turn,
    // cascading out to tests/carrier_reconnect.rs. Whether `bind` should
    // stay async is an API decision tracked separately from lint debt.
    ...
    pub async fn bind(addr: &str, cert_pem: &str, key_pem: &str) -> Result<Self, DriverError> {
        ...
        let next_peer_generation = AtomicU64::new(0);
    ...
    /// # Errors
    ///
    /// Propagates the `getsockname` failure from
    /// [`UdpSocket::local_addr`] — in practice only if the socket has
    /// been closed out from under the listener.
    ...
    /// `gridmate::server::event_bridge_loop` is the only consumer.
        ...
        let outbound_tx = {
            ...
            if handle.generation != generation {
                return Err(DriverError::Ssl(format!(
                    "stale peer session {peer_addr} ({generation:?})"
                )));
            }
    ...
    /// End one exact transport session. Dropping both channel senders wakes the
    /// peer task; generation matching prevents an old carrier task from
    /// closing a newer reconnect at the same address.
    pub(crate) fn close_peer(&self, peer_addr: SocketAddr, generation: PeerGeneration) -> bool {
        remove_peer_generation(&self.peers, peer_addr, generation)
    }
}

fn remove_peer_generation(
    peers: &PeerMap,
    peer_addr: SocketAddr,
    generation: PeerGeneration,
) -> bool {
    peers
        .remove_if(&peer_addr, |_, handle| handle.generation == generation)
        .is_some()
}
...
fn disable_udp_connreset(socket: &UdpSocket) {
    ...
    #[cfg(windows)]
    {
        ...
        // `RawSocket` is a u64 while `SOCKET` is pointer-width, so this
        // is only lossless on 64-bit. Skipping is the right failure
        // mode for a best-effort ioctl.
        let Ok(raw) = SOCKET::try_from(socket.as_raw_socket()) else {
            warn!("disable_udp_connreset: socket handle does not fit a SOCKET; skipping");
            return;
        };
        let mut disable: u32 = 0;
        let mut returned: u32 = 0;
        // WSAIoctl wants the option's length as a DWORD. `disable` is a
        // `u32`, so this is 4 — derived without a narrowing cast.
        let disable_len = u32::BITS / 8;
        let rc = unsafe {
            WSAIoctl(
                raw,
                SIO_UDP_CONNRESET,
...
async fn demux_loop(
    ...
    // Per-peer queue depth. Bounded — a slow consumer triggers
    // `.send().await` backpressure rather than unbounded memory growth.
    ...
    loop {
        ...
        let inbound = peers
            .get(&peer_addr)
            .map(|h| (h.generation, h.inbound_tx.clone()));

        if let Some((generation, inbound_tx)) = inbound {
            ...
            if inbound_tx.send(datagram).await.is_err() {
                ...
                remove_peer_generation(&peers, peer_addr, generation);
            }
            continue;
        }
        trace!(
            %peer_addr,
            bytes = datagram.len(),
            prefix = ?&datagram[..datagram.len().min(24)],
            "demux: first DTLS datagram from new peer"
        );
        ...
        let generation = PeerGeneration(next_peer_generation.fetch_add(1, Ordering::Relaxed));
        peers.insert(
            peer_addr,
            PeerHandle {
                generation,
...
/// Per-peer SSL session driver.
///
/// Handles `SSL_accept`, then loops on (inbound encrypted bytes →
/// `SSL_read` → plaintext event) and (outbound plaintext request →
/// `SSL_write` → encrypted on the wire).
async fn peer_task(
    peer_addr: SocketAddr,
    generation: PeerGeneration,
    ssl_ctx: Arc<SslContext>,
    ...
    let result = run_peer(
        peer_addr, generation, &ssl_ctx, &socket, &channels, &event_tx,
    )
    .await;
    ...
    loop {
        ...
        match event {
            Event::Inbound(Ok(datagram)) => {
                ...
                drain_decrypted(&ssl, &mut recv_ring, peer_addr, generation, event_tx).await?;
            }
            // Either channel closing means the peer is done; both ends
            // unwind the same way.
            Event::Inbound(Err(_)) | Event::Outbound(Err(_)) => return Ok(()),
            Event::Outbound(Ok(plaintext)) => {
                // `SSL_write` takes a c_int length, so an oversized
                // record is a real error rather than something to
                // truncate onto the wire.
                let len = i32::try_from(plaintext.len()).map_err(|_| {
                    format!("SSL_write: record of {} bytes exceeds i32", plaintext.len())
                })?;
                let n =
                    unsafe { openssl_sys::SSL_write(ssl.as_ptr(), plaintext.as_ptr().cast(), len) };
                if n <= 0 {
...
/// The two per-peer queues, created together in [`demux_loop`] and
/// always passed together: encrypted datagrams in, plaintext records
/// out.
struct PeerChannels {
    ...
    loop {
        ...
        if !outgoing.is_empty() {
            trace!(
                %peer_addr,
                bytes = outgoing.len(),
                prefix = ?&outgoing[..outgoing.len().min(24)],
                "DTLS handshake datagram sent"
            );
            socket
                .send_to(outgoing.as_ref(), peer_addr)
                .await
        ...
        if err != SSL_ERROR_WANT_READ && err != SSL_ERROR_WANT_WRITE {
            ...
            // wrapper yields the complete OpenSSL reason chain so callers can
            // distinguish certificate rejection, cipher mismatch, and record
            ...
            let reason = format!("SSL_accept error {err}: {joined}");
            warn!(%peer_addr, %reason, "DTLS handshake rejected");
            return Err(reason);
        }

        let Ok(datagram) = inbound_rx.recv().await else {
            return Err("inbound channel closed during handshake".into());
        };
        trace!(
            %peer_addr,
            bytes = datagram.len(),
            prefix = ?&datagram[..datagram.len().min(24)],
            "DTLS handshake datagram received"
        );
        bio_feed(read_bio, &datagram).map_err(|e| format!("bio_feed: {e}"))?;
    }
}
...
async fn drain_decrypted(
    ...
    loop {
        ...
        let plaintext = {
            ...
            // The slot is at most SSL_RECV_BUF (64 KiB), so it always
            // fits the c_int length; clamp rather than truncate so a
            // future larger ring cannot silently pass a negative size.
            let slot_len = i32::try_from(slot.len()).unwrap_or(i32::MAX);
            n = unsafe { openssl_sys::SSL_read(ssl.as_ptr(), slot.as_mut_ptr().cast(), slot_len) };
            // `n <= 0` means SSL_read returned 0 or an error code: do
            // not commit, and fall through to the error-classification
            // block below. Only a positive `n` is a byte count.
            match usize::try_from(n) {
                Ok(len) if len > 0 => recv_ring.commit(len),
                _ => Bytes::new(),
            }
        };
        if n > 0 {
