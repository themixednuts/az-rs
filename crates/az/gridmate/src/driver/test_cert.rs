...
/// # Errors
///
/// Every failure is a [`DriverError::Ssl`] carrying the failing step's
/// name and the `OpenSSL` message. The steps that can fail are RSA-2048
/// key generation, wrapping it as a `PKey`, building the `CN` subject
/// name, allocating and randomising the 159-bit serial, setting the
/// cert's version / serial / subject / issuer / public key, computing
/// the 1-day validity window, the SHA-256 self-signature, and the two
/// PEM encodings. The PEM bytes are also UTF-8 checked, which yields
/// the same variant if `OpenSSL` ever emits non-UTF-8 output.
