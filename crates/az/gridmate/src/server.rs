...
use crate::carrier::carrier_desc::CarrierRuntimeConfig;
use crate::carrier::io::ChannelTransport;
use crate::carrier::thread_message::CarrierEvent;
use crate::carrier::{CarrierDesc, CarrierImpl, DataReliability, MAX_CHANNELS, SYSTEM_CHANNEL};
use crate::driver::{DriverError, MultiPeerEvent, MultiPeerListener, PeerGeneration};
use crate::message::{EgressPath, MessageTypeId, Sendable, read_correlated_message};
...
// (No separate per-peer event type — gridmate::server emits the
...
impl PeerRegistry {
    ...
    fn register(
        ...
        if let Some(previous) = self.by_addr.insert(
            peer_addr,
            PeerEntry {
                generation,
                session,
            ...
            self.by_session.remove(&previous.session);
        }
        self.by_session.insert(session, peer_addr);
        session
    }

    fn remove(&self, peer_addr: SocketAddr, generation: PeerGeneration) -> Option<SessionId> {
        let (_, entry) = self
            .by_addr
            .remove_if(&peer_addr, |_, entry| entry.generation == generation)?;
...
impl ServerListenerHandle {
    ...
    /// # Errors
    ///
    /// Returns any error [`Self::bind_configured`] returns.
    pub async fn bind(addr: &str, cert_pem: &str, key_pem: &str) -> Result<Self, DriverError> {
        Self::bind_configured(addr, cert_pem, key_pem, CarrierDesc::default()).await
    }

    /// Bind with explicit source-compatible carrier settings.
    ///
    /// # Errors
    ///
    /// Returns [`DriverError::Io`] if `addr` cannot be parsed or the UDP
    /// socket cannot be bound, and [`DriverError::Ssl`] if `cert_pem` /
    /// `key_pem` are not valid PEM or the `SSL_CTX` rejects them — both
    /// surfaced by [`MultiPeerListener::bind`]. Also returns
    /// [`DriverError::Io`] if the bound socket's local address cannot be
    /// read back.
    ...
    /// # Errors
    ///
    /// Returns `"unknown session {session:?}"` if `session` names no
    /// peer in the registry — it never connected, or it already
    /// disconnected and was removed. Returns `"outbound queue: {e}"` if
    /// the peer's carrier driver task has exited and closed the
    /// receiving half. A full-but-live queue is not an error; the send
    ...
    /// Typed send: encode `msg` through the [`EgressPath`] `P` and
    /// enqueue on the peer's outbound queue. Mirror of
    /// [`crate::SessionServiceHandle::send`] for the server side —
    /// returns the shared [`Outgoing`] builder so server-egress and
    /// client-egress callers compose the same way. The path is a
    /// generic so dual-direction messages (`PingMsg`) can be sent
    /// either way by picking `P` at the call site.
    pub fn send<M, P>(&self, session: SessionId, msg: M) -> Outgoing<'_, M, Self, P>
    where
        M: Sendable<P>,
        P: EgressPath,
    {
...
async fn event_bridge_loop(
    ...
    let mut peer_inbound: HashMap<SocketAddr, (PeerGeneration, Sender<Bytes>)> = HashMap::new();
    ...
    loop {
        ...
        match event {
            MultiPeerEvent::Established {
                ...
                peer_inbound.insert(peer_addr, (generation, inbound_tx));
                let session = peers.register(peer_addr, generation, outbound_tx);
            ...
            MultiPeerEvent::Data {
                ...
                if let Some((current_generation, tx)) = peer_inbound.get(&peer_addr)
            ...
            MultiPeerEvent::Disconnected {
                ...
                let session = peers.remove(peer_addr, generation);
...
fn spawn_peer_carrier(
    ...
    // Carrier driver: handshake then steady-state loop. These are the
    // owned parameters, moved into the driver task — the task is the
    // only consumer, so there is nothing left here to clone for.
    let event_tx_for_driver = event_tx;
    let peers_for_driver = peers;
    let listener_for_driver = listener;
    crate::spawn::spawn_detached(async move {
        async {
            let io = ChannelTransport {
                ...
                generation,
                inbound: inbound_rx,
                listener: listener_for_driver.clone(),
            };
            let carrier = CarrierImpl::accept_configured(io, config);
            let mut connected = match carrier.ready().await {
                Ok(c) => c,
                Err(err) => {
                    warn!(?peer_addr, ?err, "carrier ready failed");
                    let session = peers_for_driver.remove(peer_addr, generation);
                    if let Some(session) = session {
        ...
        peers_for_driver.remove(peer_addr, generation);
        listener_for_driver.close_peer(peer_addr, generation);
    });
    ...
    loop {
        ...
        match action {
            Action::Inbound(Some(msg)) => {
                if !handle_inbound(peer_addr, generation, msg, event_tx, peers).await {
                    return;
                }
            }
            Action::Inbound(None) => {
...
async fn handle_inbound(
    ...
    let Some(session) = peers.session_for_generation(peer_addr, generation) else {
        return true;
    };
    match msg {
        CarrierEvent::MessageReceived { channel, data } => {
            ...
            // (sometimes) IMessage envelopes; route channel 0
            ...
            let type_index = match envelope_view.type_id {
                MessageTypeId::TypeIndex(ti) => ti,
                MessageTypeId::Uuid(_) => {
                    warn!(
                        ?peer_addr,
                        channel, "envelope identifies type by UUID; UUID dispatch not yet wired"
                    );
                    return true;
                }
            };
            ...
            event_tx
                .send(Event::TypedReceived {
                    ...
                    channel,
                    type_index,
                    correlation_id: Some(correlated.correlation_id),
                    data: body,
                })
                .await
                .is_ok()
