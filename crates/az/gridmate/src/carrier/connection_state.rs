...
///
/// Every call to `io.read()`/`io.write()` is monomorphised against the
/// concrete transport — no `dyn` dispatch and no enum-variant match
/// arms on the hot path. See [`super::io::CarrierTransport`].
pub struct ConnectionState<T: CarrierTransport> {
    ...
    /// user datagrams attempt LZ4 compression.
...
impl<T: CarrierTransport> ConnectionState<T> {
    ...
    pub fn with_send_compressed(io: T, send_compressed: bool) -> Self {
        let protocol = CarrierProtocolProfile {
            send_compressed,
            ..CarrierProtocolProfile::default()
        };
    ...
    /// toggle.
    pub const fn send_compressed(&self) -> bool {
    ...
    pub const fn set_send_compressed(&mut self, value: bool) {
    ...
    /// Format: `[version u32 BE][SM_CONNECT_REQUEST]`.
    pub fn make_connect_request_payload(&self) -> Bytes {
        ...
        msg.extend_from_slice(&[system_message::SM_CONNECT_REQUEST]);
    ...
    ///
    /// Lumberyard sends this reply reliably. The matching request remains
    /// unreliable because the initiator retries it until the acknowledgement
    ...
    /// # Errors
    ///
    /// Returns `"decode datagram: {e}"` wrapping a `DecodeError` if the
    /// header is malformed or an LZ4 payload fails to decompress;
    /// `"Duplicate datagram"` if this sequence number is already in the
    /// received history (the datagram is dropped, not a failure the
    /// caller must act on); and any error `process_system_message`
    /// raises — today only `"System message too small"` for a system
    /// frame with no message-id byte.
    ...
    pub const fn received_reliable_seq_num(&self, channel: u8) -> SequenceNumber {
        self.receiver
            .reassembler()
            .received_reliable_seq_num(channel)
    }
    ...
    fn process_system_message(&mut self, msg: &MessageData) -> Result<bool, String> {
        ...
        if msg_id > system_message::SM_CT_FIRST {
            ...
            // `SM_CT_CONN_CONTROL` carries no state the carrier tracks
            // yet, and any other carrier-thread id is accepted and
            // ignored — only the ACK frame has a handler.
            if msg_id == system_message::SM_CT_ACKS {
            ...
            // Main thread messages (SM_CONNECT_REQUEST, SM_CONNECT_ACK,
            // SM_DISCONNECT, SM_CLOCK_SYNC and anything unrecognised)
            // get no carrier-layer handling — they are forwarded
            // verbatim to the Carrier layer through the receive queue,
            // which is where SM_CONNECT_ACK is validated.
            Ok(true)
        }
    }
    ...
    /// Queue an `SM_CT_ACKS` frame if the ACK trigger has fired
    /// (`GridMate`: `m_trafficControl->IsSendAck(connection)`).
    ///
    /// `IsSendACKOnly` fires when (a) we received data since the last
    /// send, or (b) 100 ms elapsed — the keepalive ceiling
    /// (`m_lostPacketTimeoutMS / 10`).
    fn queue_ack_if_due(&mut self, now: Instant) {
    ...
    /// when `max_datagram_size` leaves no room for a payload — it must
    /// exceed [`DatagramHeader::SIZE`]. Nothing else in the send path is
    /// fallible: an empty queue and a payload where no message fits both
    /// return `Ok(None)`.
    ///
    /// # Panics
    ///
    /// Panics if LZ4 compression of the assembled payload fails. That is
    /// a `lzzzz` invariant violation, not a wire condition — the output
    /// buffer grows on demand, so the call has no failure mode that
    /// depends on peer input.
    pub fn prepare_outgoing_datagram(
        &mut self,
        max_datagram_size: usize,
    ) -> Result<Option<Bytes>, crate::GridMateError> {
        self.queue_ack_if_due(Instant::now());
        ...
        let datagram_header_size = DatagramHeader::SIZE;
        if max_datagram_size <= datagram_header_size {
            return Err(crate::GridMateError::InvalidState(format!(
                "Max datagram size too small: {max_datagram_size} <= {datagram_header_size}"
            )));
        }
        ...
        let payload_buffer = self.pack_messages(max_datagram_size, &mut dgram_data);
        ...
        // Compression begins only after the connection leaves the handshake state.
        let compression_flag = if self.send_compressed && !self.is_connecting {
            DatagramHeader::COMPRESSED
        } else {
            DatagramHeader::UNCOMPRESSED
        };
        ...
        let datagram = if compression_flag == DatagramHeader::COMPRESSED {
            ...
            if compressed.len() >= payload_data.len() {
                ...
                frame_datagram(false, datagram_seq, &payload_data)
            } else {
                frame_datagram(true, datagram_seq, &compressed)
            }
        } else {
            frame_datagram(false, datagram_seq, &payload_data)
        };
        ...
        // GridMate: dgram.m_flowControl.m_size = dataSize. The datagram is
        // bounded by `max_datagram_size`, so this cannot saturate.
        dgram_data.flow_control.size = u16::try_from(datagram.len()).unwrap_or(u16::MAX);
    ...
    /// Pack as many queued messages as fit into one datagram payload,
    /// highest priority first.
    ///
    /// Pops every message it writes off `to_send` and records the
    /// reliable ones on `dgram_data` so `process_resends` can replay
    /// them. Stops on the first message that would overflow
    /// `max_datagram_size`, leaving it (and the rest of its priority
    /// queue) for the next datagram.
    fn pack_messages(
        &mut self,
        max_datagram_size: usize,
        dgram_data: &mut DatagramData,
    ) -> WriteBuffer {
        ...
        for priority in 0..PRIORITY_MAX {
            ...
            while let Some(msg) = self.to_send[priority].front() {
                ...
                // Explicit reliable sequence IDs keep `MF_SEQUENTIAL_REL_ID` clear.
                let is_write_reliable_seq_id =
                    if is_written_first_rel_seq_num[current_channel as usize] {
                        if msg.reliability == DataReliability::Reliable {
                            ...
                            // Unreliable messages also carry an explicit reliable cursor.
                            true
                        }
                    } else {
                ...
                if msg.reliability == DataReliability::Reliable {
                    // Bodies are chunked to `mtu::MAX_MESSAGE_DATA_SIZE`
                    // and the whole datagram is capped, so this widening
                    // is total on every reachable path.
                    dgram_data.resend_data_size +=
                        u16::try_from(msg.data.len()).unwrap_or(u16::MAX);
    ...
    pub fn queue_message(&mut self, message: MessageData, priority: usize) {
        ...
        if data_len <= mtu::MAX_MESSAGE_DATA_SIZE {
            ...
            // `num_chunks <= MAX_NUM_CHUNKS` (32767) was checked above, so
            // the chunk counter fits the u16 the wire carries.
            let mut chunks_remaining = u16::try_from(num_chunks).unwrap_or(u16::MAX);
            ...
            while !remaining_data.is_empty() {
                ...
                self.queue_message_chunk(chunk_msg, priority, chunks_remaining);
    ...
    pub fn process_resends(&mut self) -> usize {
        ...
        const LOST_PACKET_TIMEOUT: std::time::Duration = std::time::Duration::from_secs(1);
        ...
        while let Some(front) = self.send_datagrams.front() {
            ...
            // This datagram is lost. Extract it. `front()` just handed us
            // one, so the `else` arm is unreachable — written as a `break`
            // rather than an `unwrap` so the queue's shape, not a panic,
            // decides when the loop ends.
            let Some(mut dgram) = self.send_datagrams.pop_front() else {
                break;
            };
...
/// Assemble a carrier datagram using the current four-byte header.
fn frame_datagram(is_compressed: bool, seq: SequenceNumber, body: &[u8]) -> Bytes {
    let mut buffer = Vec::with_capacity(DatagramHeader::SIZE + body.len());
    buffer.push(if is_compressed {
        DatagramHeader::COMPRESSED
    } else {
        DatagramHeader::UNCOMPRESSED
    });
    buffer.push(DatagramHeader::HAS_COMPRESSOR);
    buffer.extend_from_slice(&seq.get().to_be_bytes());
...
mod tests {
    ...
    fn explicit_profile_drives_connect_request_and_ack_payloads() {
        ...
        assert_eq!(ack.reliability, DataReliability::Reliable);
        assert_eq!(
            ack.data.as_ref(),
    ...
    #[test]
    fn current_format_frames_the_fixed_four_byte_header() {
        let bytes = frame_datagram(false, SequenceNumber::from(0x1234), &[0xaa]);
