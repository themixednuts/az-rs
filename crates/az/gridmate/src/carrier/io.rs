...
#[cfg(feature = "server")]
use crate::driver::{MultiPeerListener, PeerGeneration};
#[cfg(feature = "server")]
...
pub trait CarrierTransport: Send + 'static {
    ...
    /// # Errors
    ///
    /// Returns [`DriverError::ConnectionClosed`] once the peer's
    /// inbound path is gone — a closed demux channel for
    /// `ChannelTransport`, a shut-down socket for `DtlsTransport`. A
    /// DTLS transport additionally surfaces [`DriverError::Ssl`] for a
    /// decrypt failure and [`DriverError::Io`] for a socket-level read
    /// error. An empty queue is `Ok(None)`, never an error.
...
/// UDP socket used by initiating clients.
#[cfg(feature = "client")]
...
/// Responder-side transport backed by the shared multi-peer listener.
///
/// Plaintext arrives via `inbound` (fed by [`MultiPeerListener`]'s
/// demux loop) and outbound carrier datagrams go directly into the
/// listener's per-peer `SSL_write` queue via
/// `listener.send_to(peer_addr, ...)`. One task per peer is
/// eliminated by skipping any intermediate forwarder hop.
#[cfg(feature = "server")]
pub struct ChannelTransport {
    pub peer_addr: SocketAddr,
    pub(crate) generation: PeerGeneration,
...
impl CarrierTransport for ChannelTransport {
    ...
    async fn write(&mut self, data: Bytes) -> Result<usize, DriverError> {
        let len = data.len();
        self.listener
            .send_to(self.peer_addr, self.generation, data)
            .await?;
        Ok(len)
    }
}
