// MessageData is the smallest unit transported by the carrier.
// Lumberyard reference: dev/Code/Framework/GridMate/GridMate/Carrier/Carrier.cpp.

use super::types::{MAX_CHANNELS, MAX_CHANNELS_U8, SequenceNumber};
...
/// Delivery priority corresponding to Lumberyard `GridMate`'s
...
impl MessageData {
    ...
    pub fn read_header(
        ...
        if *current_channel >= MAX_CHANNELS_U8 {
            return None;
        }
    ...
    /// Payload length as it appears in the carrier header's 16-bit
    /// `data_size` field.
    ///
    /// Returns `None` when the payload is longer than [`u16::MAX`] and so
    /// cannot be described by the header at all. The send path chunks
    /// payloads to
    /// [`MAX_MESSAGE_DATA_SIZE`](super::mtu::MAX_MESSAGE_DATA_SIZE) — about
    /// 1.1 KB — so every message the carrier actually emits is `Some`.
    #[must_use]
    pub fn data_size(&self) -> Option<u16> {
        u16::try_from(self.data.len()).ok()
    }
}
