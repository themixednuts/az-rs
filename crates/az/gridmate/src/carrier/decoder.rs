...
//! capture and replay tooling otherwise had to
...
pub struct DecodedDatagram {
    /// Carrier datagram header parsed from the current wire format.
...
pub enum DecodeError {
    ...
    /// LZ4 decompression failed for a payload marked compressed.
...
/// # Errors
///
/// Returns [`DecodeError::Header`] if the header is truncated or its marker
/// bytes do not match the current wire format, and [`DecodeError::Decompress`] if a
/// compressed payload fails LZ4 decompression or expands past
/// `MAX_DECOMPRESSED_BYTES`. [`DecodeError::FrameParseStop`] is part of
/// the vocabulary for callers that re-check the frame walk; this
/// function itself stops the walk silently and returns the frames it
/// managed to parse.
///
...
pub fn decode_datagram(mut data: Bytes) -> Result<DecodedDatagram, DecodeError> {
    ...
    // Drop the header prefix in place. `Bytes::advance` only moves the
    // view's start offset, so the remaining payload stays zero-copy and
    // the uncompressed branch below can hand `data` straight out.
    data.advance(payload_start);

    let payload: Bytes = if header.is_compressed {
        let mut scratch = vec![0u8; MAX_DECOMPRESSED_BYTES];
        let n = lzzzz::lz4::decompress_partial(data.as_ref(), &mut scratch, MAX_DECOMPRESSED_BYTES)
        ...
        // Zero-copy: the advanced view *is* the payload.
        data
    };
