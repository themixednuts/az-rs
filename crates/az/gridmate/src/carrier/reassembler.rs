...
impl Reassembler {
    ...
    /// dropping the borrow.
    pub const fn drain(&mut self) -> Drain<'_> {
        Drain {
            reassembler: self,
            channel: 0,
        }
    ...
    /// chunks).
    fn next_ready(&mut self, channel: u8) -> Option<MessageData> {
        let idx = usize::from(channel);
        // Peek the head and decide whether it's deliverable.
        let head = self.staging[idx].front()?;
        ...
        let last_rel = self.received_reliable_seq_num[idx];
        ...
        if head_reliable {
            ...
            let dist = last_rel.sequential_distance_to(head_send_rel);
            if dist != 1 {
                return None;
            }
            if head_num_chunks > 1 {
                if self.staging[idx].len() < head_num_chunks {
                    return None;
                }
        ...
        let msg = self.staging[idx].pop_front()?;
        if msg.reliability == DataReliability::Reliable {
            self.received_reliable_seq_num[idx] = msg.send_reliable_seq_num;
        }
        self.received_seq_num[idx] = msg.sequence_number;
        Some(msg)
    }
    ...
    fn chunks_ready(&self, channel: u8, num_chunks: usize) -> bool {
        let idx = usize::from(channel);
        if self.staging[idx].len() < num_chunks {
            return false;
        }
        let mut iter = self.staging[idx].iter();
        ...
        for chunk in iter.take(num_chunks - 1) {
            ...
            let dist = prev_rel_seq.sequential_distance_to(chunk.send_reliable_seq_num);
            if dist != 1 {
                return false;
            }
    ...
    /// gate moves forward.
    fn assemble_chunks(&mut self, channel: u8, num_chunks: usize) -> Option<MessageData> {
        let idx = usize::from(channel);
        let total_size: usize = self.staging[idx]
            .iter()
            .take(num_chunks)
            .map(|m| m.data.len())
            .sum();
        ...
        for _ in 0..num_chunks {
            let chunk = self.staging[idx].pop_front()?;
        ...
        self.received_reliable_seq_num[idx] = last_rel;
        self.received_seq_num[idx] = last_seq;
...
impl Iterator for Drain<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while usize::from(self.channel) < MAX_CHANNELS {
            if let Some(msg) = self.reassembler.next_ready(self.channel) {
                return Some((self.channel, msg));
            }
            self.channel += 1;
        }
        None
