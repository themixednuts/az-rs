...
/// These match OpenSSL's `SSL3_RT`_* constants from openssl/ssl3.h
#[repr(u8)]
...
/// These match OpenSSL's `SSL3_MT`_* constants from openssl/ssl3.h
#[repr(u8)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub fn is_handshake(data: &[u8]) -> bool {
    RecordHeader::parse(data).is_some_and(|header| header.is_handshake())
}
...
pub fn print_client_hello_details(data: &[u8]) -> String {
    ...
    if let Some(record_hdr) = RecordHeader::parse(data) {
        ...
        if data.len() >= 25 {
            ...
            details.push(format!("Version: 0x{version:04x}"));
