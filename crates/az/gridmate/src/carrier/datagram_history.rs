...
pub(super) const fn sequence_number_sequential_distance(
    from: SequenceNumber,
    to: SequenceNumber,
) -> u16 {
...
impl DatagramHistoryList {
    ...
    /// [`Self::MAX_ELEMENTS`] in the sequence-number width. Kept as its
    /// own constant so the eviction-window arithmetic in
    /// [`Self::insert`] subtracts in `u16` instead of truncating a
    /// `usize`.
    const MAX_ELEMENTS_U16: u16 = (Self::MAX_NUM_BYTES as u16) * 8; // 512
    ...
    pub const fn inc(n: &mut usize) {
        *n = Self::add(*n);
    }

    /// Decrement an offset in-place with wrapping.
    pub const fn dec(n: &mut usize) {
        *n = Self::sub(*n);
    }
    ...
    pub fn get_for_ack(&mut self, offset: usize) -> (SequenceNumber, usize) {
        ...
        self.elements[offset].num_acks_sent += 1;
        if self.elements[offset].num_acks_sent >= i32::from(Self::MAX_NUM_ACKS) {
    ...
    pub fn insert(&mut self, id: SequenceNumber) -> bool {
        ...
        if self.num_active_elements != 0 {
            ...
            if seq_less_than(last_seq, id) {
                ...
                let new_first_seq =
                    SequenceNumber::from(id.get().wrapping_sub(Self::MAX_ELEMENTS_U16 - 1));
...
pub(super) fn write_ack_history(
    ...
    if dist == 0 {
    ...
    } else if usize::from(dist) == num_datagrams_to_ack - 1 {
        ...
        // `dist` is a u16, so `dist / 8` rounded up can exceed `u8::MAX`
        // — but the wire caps the bitmap at MAX_NUM_BYTES (64), and
        // anything that overflows a u8 is far past that cap, so the
        // saturating branch and the clamp agree.
        let num_history_bytes = u8::try_from(dist.div_ceil(8))
            .unwrap_or(DatagramHistoryList::MAX_NUM_BYTES)
            .min(DatagramHistoryList::MAX_NUM_BYTES);
        ack_flags |= 0x80; // AHF_BITS
        ack_flags |= num_history_bytes;
        history_bits.resize(usize::from(num_history_bytes), 0);
