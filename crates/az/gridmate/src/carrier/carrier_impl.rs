...
/// Concrete carrier implementation, as a type-level state machine.
///
/// Communicates with the background `CarrierDriver` over channels; the
/// connection state is encoded in the `State` type parameter, so an
/// operation that only makes sense on a live connection cannot be
/// named on a `Disconnected` value. `Disconnected` is the default
/// parameter for backward compatibility with call sites that wrote
    ...
    State: Send + Sync,
{
    ...
    #[cfg(feature = "client")]
    pub(crate) const fn take_event_receiver(&mut self) -> Option<Receiver<CarrierEvent>> {
    ...
    /// # Errors
    ///
    /// Returns [`GridMateError::Channel`] if the command channel to
    /// the carrier driver has been closed — the driver task exited
    /// (shutdown, transport failure) and no further sends can be
    /// queued. A live-but-full channel does not error; the send
    /// awaits backpressure instead.
    pub async fn send(
        &mut self,
...
impl CarrierImpl<Disconnected> {
    ...
    /// # Errors
    ///
    /// Returns [`GridMateError::Driver`] if the DTLS connection to
    /// [`CarrierDesc::server_address`] cannot be established — the
    /// address is unreachable, the handshake times out, or the server's
    /// certificate fails the `ca_cert` check. Once the transport is up
    /// the remaining setup is infallible; the carrier returns in the
    /// `Connecting` state and the handshake itself completes
    /// asynchronously on the driver task.
    #[cfg(feature = "client")]
    pub async fn new(desc: CarrierDesc) -> Result<CarrierImpl<Connecting>> {
        ...
        let config = desc.runtime_config();
        let protocol = config.protocol;
        let conn = ConnectionState::with_protocol_profile(DtlsTransport(dtls), protocol);
        let carrier = Self::start_with(conn, CarrierRole::Initiator, config.disconnect_timeout);
        Self::connect_request(protocol);
        Ok(carrier)
    }
    ...
    /// # Errors
    ///
    /// Never fails today: unlike the initiator-side `new`, the
    /// responder path does no I/O at construction — it only wires up
    /// channels and spawns the driver, and the handshake it is
    /// accepting has already arrived. The `Result` is kept so this
    /// stays signature-compatible with `new` (which returns
    /// [`GridMateError::Driver`] on a failed DTLS connect) and so a
    /// future transport that validates `io` up front does not break
    /// callers.
    pub fn accept<T: CarrierTransport>(io: T) -> Result<CarrierImpl<Connecting>> {
        Ok(Self::accept_configured(
            io,
            CarrierDesc::default().runtime_config(),
        ))
    }

    /// Responder-side carrier with the listener's source-compatible settings.
    pub(crate) fn accept_configured<T: CarrierTransport>(
        io: T,
        config: CarrierRuntimeConfig,
    ) -> CarrierImpl<Connecting> {
        let conn = ConnectionState::with_protocol_profile(io, config.protocol);
        Self::start_with(conn, CarrierRole::Responder, config.disconnect_timeout)
    }
...
where
    State: Send + Sync,
{
    /// Note that the connection handshake is starting.
    ///
    /// The carrier thread's retry loop handles all `SM_CONNECT_REQUEST`
    /// messages with exponential backoff (10, 20, 40, 80... up to
    /// 1000ms), and its `handshake_retry_time` is initialised to fire
    /// immediately — so the first `SM_CONNECT_REQUEST` goes out from
    /// `handle_handshake_retry()` without anything being sent here.
    /// This is purely the log line marking that point, which is why it
    /// takes no receiver, awaits nothing, and cannot fail.
    #[cfg(feature = "client")]
    fn connect_request(protocol: CarrierProtocolProfile) {
        debug!(
    ...
    async fn send_message(&self, msg: MessageData, priority: usize) -> Result<()> {
        self.inner()
            .to_carrier_tx
...
impl CarrierImpl<Connecting> {
    ...
    /// # Errors
    ///
    /// Returns [`GridMateError::ConnectionClosed`] if the event
    /// receiver was already taken by another consumer, or if the
    /// carrier driver dropped its sender before announcing a
    /// connection. Returns [`GridMateError::Carrier`] if the driver
    /// reported `Disconnected` (carrying the
    /// [`DisconnectReason`](super::types::DisconnectReason)) or an
    /// `Error` event (carrying the driver's description) while we were
        ...
        loop {
            match rx.recv().await {
                ...
                Ok(CarrierEvent::Disconnected { reason }) => {
                    return Err(GridMateError::Carrier(format!("Disconnected: {reason:?}")));
                }
...
impl CarrierImpl<Connected> {
    ...
    /// # Errors
    ///
    /// Never fails today. The `Disconnect` command is sent
    /// best-effort — a closed command channel means the driver task is
    /// already gone, which is the outcome this call was asking for —
    /// and the typestate transition that follows cannot fail. The
    /// `Result` is kept so callers keep the `?` that a future
    /// flush-before-close step will need.
