...
impl SequenceNumber {
    ...
    #[must_use]
    pub const fn wrapping_sub(self, other: Self) -> u16 {
        other.0.wrapping_sub(self.0)
    }

    #[must_use]
    pub const fn sequential_distance_to(self, other: Self) -> u16 {
        other.0.wrapping_sub(self.0)
    }
}
...
/// [`MAX_CHANNELS`] in the wire's channel-id width.
///
/// Loops that yield a channel id count up to this instead of narrowing
/// [`MAX_CHANNELS`] with a cast. The two are pinned together below.
pub const MAX_CHANNELS_U8: u8 = 4;

const _: () = assert!(MAX_CHANNELS == MAX_CHANNELS_U8 as usize);
...
impl ChannelId {
    ...
    #[must_use]
    pub const fn new_unchecked(channel: u8) -> Self {
        Self(channel)
    }
