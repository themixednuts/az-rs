//! `MB::ReplicatedState` — variable filter-group replicated state base class.
...
/// RTTI type id for `MB::ReplicatedState`
/// (`MF_RTTI(ReplicatedState, "261f7815-2be0-44f5-be4a-e8070677a57b")`).
pub const REPLICATED_STATE_TYPE_ID: Uuid = Uuid::from_u128(0x261f7815_2be0_44f5_be4a_e8070677a57b);

/// Vendor constants forwarded from `Amazon::Hub::ReplicatedStateConstants`.
#[allow(non_snake_case)]
pub mod ReplicatedStateConstants {
    /// Upper bound used by `MB::ReplicatedState` default-bit bitsets.
    pub const MAX_FIELDS_PER_REPLICATED_STATE: u8 = 64;
    /// Native `AddFilterGroup` allows group indices `0..=9`.
    pub const MAX_FILTERGROUPS_PER_REPLICATED_STATE: u8 = 10;
}

/// `MB::FilterValue` — refcounted client-filter entry stored in a filter group.
...
/// Source `ReplicatedState::FieldInfo`.
...
/// Mutable source `ReplicatedState::FieldInfo` view.
...
/// Source `ReplicatedState::DefaultBits`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
...
/// Accumulator for source `ReplicatedState::MergeAndUpdateSequence`.
...
/// Source `ReplicatedState::FilterGroup` bookkeeping that does not borrow
...
impl ReplicatedFilterGroup {
    ...
    #[must_use]
    pub const fn explicit_client_targets_last_modified(&self) -> SequenceNumber {
        self.explicit_client_targets.last_modified()
    }

    #[must_use]
    pub const fn default_bits_last_modified(&self) -> SequenceNumber {
        self.default_bits.last_modified()
    }
}

/// Rust port of `MB::ReplicatedState`.
///
/// Variable-length sibling of [`super::FixedReplicatedState`]. Concrete Rust
/// states embed this base and generate their source field-registration surface
...
impl ReplicatedState {
    /// Source default constructor: one default filter group at index 0.
    #[must_use]
    pub fn new() -> Self {
        Self::with_reserved_groups(1)
    }

    /// Source `ReplicatedState(size_t reserveGroups)`.
    ///
    /// Passing zero deliberately skips the default group for ALC-style states
    ...
    ///
    /// Source constructors call `AddFilterGroup` while registering fields.
    ...
    #[must_use]
    pub const fn num_filter_groups(&self) -> usize {
        self.filter_groups.len()
    }

    /// Source `AddFilterGroup`.
    pub fn add_filter_group(&mut self) -> usize {
    ...
    pub const fn register_attribute(&mut self) {
        self.attr_count += 1;
    }
    ...
    /// Source `AddClientToReplicationWhitelist`: entries are ref-counted.
    pub fn add_client_to_replication_whitelist(&mut self, client_id: u64, group_idx: usize) {
    ...
    pub fn remove_client_from_replication_whitelist(&mut self, client_id: u64, group_idx: usize) {
        let Some(group) = self.filter_groups.get_mut(group_idx) else {
            return;
        };
        group.explicit_client_targets.access(|targets| {
    ...
    pub fn clear_replication_whitelist(&mut self, group_idx: usize) {
    ...
    /// Source `ApplyDefaultBits`.
    pub fn apply_default_bits(&self, group_idx: usize, fields: &mut [ReplicatedFieldInfoMut<'_>]) {
        let Some(group) = self.filter_groups.get(group_idx) else {
            return;
        };
    ...
    /// Source `MarshalAttributes` for built-in filter-group attributes.
    pub fn marshal_filter_group_attributes(
        &self,
        baseline: SequenceNumber,
        wb: &mut WriteBuffer,
    ) -> bool {
        self.marshal_registered_attributes(baseline, &[], wb)
    }

    /// Source `MarshalAttributes` over filter-group bookkeeping followed by
    /// the concrete state's `RegisterAttribute` fields.
    pub fn marshal_registered_attributes(
        &self,
        baseline: SequenceNumber,
        attributes: &[ReplicatedFieldInfo<'_>],
        wb: &mut WriteBuffer,
    ) -> bool {
        let filter_attribute_count = self.filter_groups.len().saturating_mul(2);
        let attr_count = filter_attribute_count.saturating_add(attributes.len());
        debug_assert!(
        ...
        for (attribute_idx, attribute) in attributes.iter().enumerate() {
            debug_assert!(!attribute.is_filter_group);
            if attribute.handler.is_dirty(baseline) {
                field_mask |= 1u64 << (filter_attribute_count + attribute_idx);
            }
        }
        ...
        for (attribute_idx, attribute) in attributes.iter().enumerate() {
            if (field_mask & (1u64 << (filter_attribute_count + attribute_idx))) != 0 {
                attribute.handler.marshal_field_since(wb, baseline);
            }
        }
        true
    }

    /// Source `UnmarshalAttributes` for built-in filter-group attributes.
    ///
    /// # Errors
    ///
    /// Returns any error [`Self::unmarshal_registered_attributes`] returns.
    pub fn unmarshal_filter_group_attributes(
        &mut self,
        rb: &mut ReadBuffer,
    ) -> Result<bool, MarshalerError> {
        self.unmarshal_registered_attributes(&mut [], rb)
    }

    /// Source `UnmarshalAttributes` over filter-group bookkeeping followed by
    /// the concrete state's `RegisterAttribute` fields.
    ///
    /// # Errors
    ///
    /// Returns [`MarshalerError::BufferUnderrun`] if the VLQ field mask or any
    /// field the mask selects runs past the end of `rb`, plus whatever the
    /// individual field handlers raise for their own payloads (for example
    /// [`MarshalerError::ContainerOverflow`] on an over-long client-target
    /// list).
    pub fn unmarshal_registered_attributes(
        &mut self,
        attributes: &mut [ReplicatedFieldInfoMut<'_>],
        rb: &mut ReadBuffer,
    ) -> Result<bool, MarshalerError> {
        ...
        for (attribute_idx, attribute) in attributes.iter_mut().enumerate() {
            debug_assert!(!attribute.is_filter_group);
            if (field_mask & (1u64 << (filter_attribute_count + attribute_idx))) != 0 {
                attribute.handler.unmarshal_field(rb)?;
            }
        }
        Ok(true)
    }
    ...
    pub fn marshal_filter_group_attribute_metadata(&self, wb: &mut WriteBuffer) -> bool {
        self.marshal_registered_attribute_metadata(&[], wb)
    }

    /// Source attribute portion of `MarshalFieldMetadata`.
    pub fn marshal_registered_attribute_metadata(
        &self,
        attributes: &[ReplicatedFieldInfo<'_>],
        wb: &mut WriteBuffer,
    ) -> bool {
        ...
        for attribute in attributes {
            attribute.handler.last_modified().marshal(wb);
        }
        true
    }

    /// Source `UnmarshalFieldMetadata` for built-in filter-group attributes.
    ///
    /// # Errors
    ///
    /// Returns any error [`Self::unmarshal_registered_attribute_metadata`]
    /// returns.
    pub fn unmarshal_filter_group_attribute_metadata(
        &mut self,
        rb: &mut ReadBuffer,
    ) -> Result<bool, MarshalerError> {
        self.unmarshal_registered_attribute_metadata(&mut [], rb)
    }

    /// Source attribute portion of `UnmarshalFieldMetadata`.
    ///
    /// # Errors
    ///
    /// Returns [`MarshalerError::BufferUnderrun`] if `rb` ends before every
    /// filter group and attribute has yielded its `last_modified`
    /// [`SequenceNumber`].
    pub fn unmarshal_registered_attribute_metadata(
        &mut self,
        attributes: &mut [ReplicatedFieldInfoMut<'_>],
        rb: &mut ReadBuffer,
    ) -> Result<bool, MarshalerError> {
        ...
        for attribute in attributes {
            attribute
                .handler
                .set_last_modified(SequenceNumber::unmarshal(rb)?);
        }
        Ok(true)
    }

    /// Merge concrete `RegisterAttribute` fields with the same replicated
    /// field semantics used by content fields and filter-group attributes.
    pub fn merge_registered_attributes(
        old_attributes: &[ReplicatedFieldInfo<'_>],
        new_attributes: &mut [ReplicatedFieldInfoMut<'_>],
        merged_attributes: &mut [ReplicatedFieldInfoMut<'_>],
        seq: SequenceNumber,
        inherit_previous_network_data_status: bool,
        outcome: &mut ReplicatedMergeOutcome,
    ) {
        debug_assert_eq!(old_attributes.len(), new_attributes.len());
        debug_assert_eq!(old_attributes.len(), merged_attributes.len());
        for ((old_attribute, new_attribute), merged_attribute) in old_attributes
            .iter()
            .zip(new_attributes.iter_mut())
            .zip(merged_attributes.iter_mut())
        {
            debug_assert_eq!(old_attribute.name, new_attribute.name);
            debug_assert_eq!(old_attribute.name, merged_attribute.name);
            outcome.detected_new_data_in_last_merge |=
                merged_attribute.handler.merge_and_update_sequence(
                    old_attribute.handler,
                    new_attribute.handler,
                    seq,
                    inherit_previous_network_data_status,
                );
            outcome.last_modified = outcome
                .last_modified
                .max(merged_attribute.handler.last_modified());
            outcome.has_new_network_data |= merged_attribute.handler.has_new_network_data();
        }
    }

    /// Clear the network-data marker on concrete `RegisterAttribute` fields.
    pub fn reset_registered_attribute_network_data(attributes: &mut [ReplicatedFieldInfoMut<'_>]) {
        for attribute in attributes {
            attribute.handler.reset_has_new_network_data();
        }
    }

    /// Source `SanityCheckFieldCount`.
    #[must_use]
    pub const fn sanity_check_field_count(&self, field_group: usize, field_count: usize) -> bool {
...
mod tests {
    ...
    fn missing_generated_groups_behave_like_empty_whitelists() {
        ...
        assert!(header.should_send_to_client(7, 2));

        header.add_client_to_replication_whitelist(7, 2);

        assert_eq!(header.num_filter_groups(), 3);
        assert!(header.should_send_to_client(7, 2));
        assert!(!header.should_send_to_client(8, 2));
    }
}
