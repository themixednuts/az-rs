...
/// instead of constructing sentinel-shaped [`Seq`](Self::Seq) variants.
///
/// The wire form is `Option<VLQ-u64>`: `Invalid` is `None` (`0x00`),
/// `ValidNonSequence` is `Some(0)` (`0x01 0x00`), and `Seq(n)` is `Some(n)`
/// (`0x01` followed by the VLQ-u64 sequence value).
...
impl SequenceNumber {
    ...
    /// `m_lastModified.IsValid()`.
    #[must_use]
    pub const fn is_valid(self) -> bool {
        self.wire_value() != Self::INVALID_SEQUENCE_VALUE
    }
...
impl PartialEq for SequenceNumber {
    fn eq(&self, other: &Self) -> bool {
        self.wire_value() == other.wire_value()
    }
}
...
impl Ord for SequenceNumber {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        let left = self.wire_value();
        let right = other.wire_value();
...
impl Marshaler for SequenceNumber {
    fn marshal(&self, wb: &mut WriteBuffer) {
        let raw = match Self::from_raw(self.wire_value()) {
            Self::Invalid => None,
    ...
    fn unmarshal(rb: &mut ReadBuffer) -> Result<Self, MarshalerError> {
        Ok(Option::<VlqU64>::unmarshal(rb)?.map_or(Self::Invalid, Into::into))
    }
}
