...
impl<Reliability, Bandwidth, Payload> StateBundleBuilder<Reliability, Bandwidth, Payload> {
    ...
    /// Attach an already validated replication-control payload.
    ///
    /// This is useful when the caller builds stop/pause partitions atomically
    /// before constructing the enclosing bundle.
    #[must_use]
    pub fn with_replication_control(mut self, control: ReplicationControlData) -> Self {
        self.replication_control = Some(control);
        self
    }
    ...
    fn replication_control_mut(&mut self) -> &mut ReplicationControlData {
