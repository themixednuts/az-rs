...
use super::{
    DynFragment, ReplicatedStateBundle, ReplicatedStateBundleView, StateFragmentHeaderSpan,
    StateRecordHeader, consume_fragment_contents_by_type_index, decode_state_fragment_contents,
...
impl StateFragmentView<'_> {
    ...
    /// # Errors
    ///
    /// Returns any error reported by the fragment content decoder.
...
mod tests {
    ...
    use crate::{
        az::Class,
        hub::{DynFragment, Fragment, FragmentBase, StateFragmentTypeId, StateRecordWriter},
    ...
    fn fragment_iterator_yields_borrowed_bodies_and_headers() {
        ...
        assert_eq!(fragments[0].record.interest_id, 7);
        assert_eq!(fragments[0].header.fragment_key, 3);
        assert_eq!(
            fragments[0].header.type_id,
            StateFragmentTypeId::RawUuid(<ByteFragment as AzTypeInfo>::TYPE_ID)
        );
        assert_eq!(fragments[0].body, &[0xcc]);
        assert_eq!(fragments[0].decode::<ByteFragment>().unwrap().value, 0xcc);
        assert_eq!(
            fragments[0].header.type_id.uuid(),
            Some(<ByteFragment as AzTypeInfo>::TYPE_ID)
        );
    }
}
