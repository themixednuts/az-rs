//! Wire capture sidecar. When a capture directory is configured, every
//! outbound and inbound envelope is persisted to disk alongside a JSONL
//! metadata trail.
//!
...
/// Cached capture-directory environment variable. Resolved at first access
/// and never read again, which avoids a syscall and allocation per message.
pub fn capture_dir() -> Option<&'static str> {
    static DIR: OnceLock<Option<String>> = OnceLock::new();
    DIR.get_or_init(|| std::env::var("AZOTH_GRIDMATE_CAPTURE_DIR").ok())
        .as_deref()
}
...
impl CaptureCounters {
    /// Capture one outbound message. Does nothing when capture output is
    ...
    pub fn capture_send(
        ...
        let type_part = type_index.map_or_else(
            || "Tunknown".to_string(),
            |type_index| format!("T0x{type_index:x}"),
        );
            ...
            .open(&log_path)
        {
            ...
            let type_json =
                type_index.map_or_else(|| "null".to_string(), |type_index| type_index.to_string());
            let _ = writeln!(
                f,
    ...
    /// Capture one inbound envelope: full envelope bytes, a first-of-type body
    /// fixture, and a JSONL line. Does nothing when capture output is disabled.
    pub fn capture_recv(
        &mut self,
        session_id: SessionId,
        type_index: u32,
