//! Compose-seam tests for `azoth.roads-and-rivers`.
//!
...
use az_gem_roads_and_rivers::{RoadComponent, package_contribution, prefab_types_contribution};
use az_prefab::PrefabType;
use azoth_roads_and_rivers_ids::{
    GEM,
...
fn the_package_composes_without_claiming_a_registry() {
    for role in PACKAGE_ROLES {
        ...
        composer
            ...
            .expect("roads-and-rivers declares no capability floor");
...
#[test]
fn the_prefab_bundle_carries_the_road_component() {
...
fn the_two_contributions_compose_together_at_project_host() {
    ...
    assert_eq!(report.entries.len(), 1, "the road component, claimed once");
    assert!(
