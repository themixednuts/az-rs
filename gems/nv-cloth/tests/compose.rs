//! Compose-seam test for `azoth.nv-cloth`.
//!
...
use az_gem_nv_cloth::package_contribution;
use azoth_nv_cloth_ids::{GEM, contributions::PACKAGE};
...
fn every_declared_role_composes_it_without_a_refusal() {
    for role in ROLES {
        ...
        composer
            ...
            .expect("nv-cloth declares no capability floor");

        let report = composer.finalize().expect("composition is valid");
        assert!(report.refusals.is_empty(), "refused by `{role}`");
        assert!(
            report.entries.is_empty(),
            "the cloth asset and format entries belong to `az-nv-cloth`, composed by the engine"
        );
    }
}
