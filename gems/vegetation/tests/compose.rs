//! Compose-seam tests for `azoth.vegetation`.
//!
...
use az_gem_vegetation::{
    SpawnerComponent, SpawnerConfig, asset_types, package_contribution, prefab_types,
    prefab_types_contribution, types,
};
use az_prefab::PrefabType;
use azoth_vegetation_ids::{
    GEM,
...
#[test]
fn the_package_carries_the_spawner_types_and_the_distribution_asset() {
    for role in PACKAGE_ROLES {
        let mut composer = Composer::new(role);
        composer
            .add(package_contribution(), ProductActivation::default())
            .expect("vegetation declares no capability floor");
        ...
        assert_eq!(
            az_core::rtti::composed(composer.registries())
                ...
                .collect::<Vec<_>>(),
            vec![SpawnerConfig::TYPE_ID, SpawnerComponent::TYPE_ID],
        );
    }
}
