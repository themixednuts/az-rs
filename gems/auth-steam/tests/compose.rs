//! Compose-seam test for `azoth.auth.steam`.
//!
...
fn the_provider_composes_into_the_named_service_and_claims_nothing() {
    ...
    let instance = composer
        .add(
            az_gem_auth_steam::provider_contribution(),
            ProductActivation::default(),
        )
        .expect("the provider declares no capability floor");
    assert_eq!(instance.gem.as_str(), "azoth.auth.steam");
...
/// Composition is not wiring: Steam still needs a live ticket verifier from the
/// platform gem, and the entry point still fails closed without one.
#[test]
fn composition_does_not_stand_in_for_the_missing_platform_runtime() {
    ...
    let context = az_gem_auth::AuthProviderContext::new(
        ...
        az_gem_auth::ProviderId::new(az_gem_auth_steam::PROVIDER_ID),
    );
    assert!(matches!(
        az_gem_auth_steam::auth_provider(&context),
